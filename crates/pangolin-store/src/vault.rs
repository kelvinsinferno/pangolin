...
impl Vault {
    ...
    /// **MVP-4-L L-0a-2.2 recoverer role.** Persist a per-attempt
    /// ephemeral X25519 keypair sealed-at-rest under the active session's
    /// VDK column-AEAD (AAD binds target vault id + attempt nonce —
    /// anti-transplant).
    ///
    /// The keypair is pre-generated by the caller via
    /// [`pangolin_crypto::share_transport::generate_recoverer_keypair`] —
    /// that split exists because the FFI's `vault_initiate_recovery`
    /// needs to know the secret + pubkey BEFORE broadcasting (the pubkey
    /// becomes the on-chain `recipientCommitment`), but the actual
    /// `attempt_nonce` (which the AAD binds) only comes back from the
    /// contract's `RecoveryInitiated` event AFTER a successful broadcast.
    ///
    /// `INSERT OR REPLACE` semantics on `target_vault_id`: a fresh
    /// attempt against the same target overwrites the prior row's
    /// ciphertext atomically (Decision A: ephemeral per attempt).
    ///
    /// Session-gated (Active). Mutating: opens a single transaction +
    /// commits.
    ///
    /// # Errors
    ///
    /// [`StoreError::NotUnlocked`] if no Active session;
    /// [`StoreError::Sqlite`] / AEAD-seal errors on the persistence path.
    pub fn persist_recovery_recipient(
        &mut self,
        target_vault_id: [u8; pangolin_crypto::keys::VAULT_ID_LEN],
        attempt_nonce: u64,
        secret_bytes: &[u8; 32],
        public_bytes: &[u8; 32],
    ) -> Result<()> {
        ...
        let now = std::time::SystemTime::now()
            ...
            .unwrap_or(0);
        let tx = self.conn.unchecked_transaction()?;
        crate::recovery_recipient::write_recipient_tx(
            &tx,
            vdk_aead,
            &target_vault_id,
            attempt_nonce,
    ...
    /// **MVP-4-L L-0a-2.2 recoverer role.** Read the non-secret recipient
    /// identity for `target_vault_id` if an active recovery attempt has
    /// recorded one. Returns `None` if no row exists.
    ///
    /// Loaded-only (any non-placeholder handle — no VDK access required;
    /// the secret is NOT decrypted on this path).
    ///
    /// # Errors
    ///
    /// [`StoreError::Sqlite`] / [`StoreError::Corrupted`] on a malformed
    /// row.
    pub fn recovery_recipient_pubkey(
        &self,
        target_vault_id: [u8; pangolin_crypto::keys::VAULT_ID_LEN],
    ) -> Result<Option<crate::recovery_recipient::StoredRecipientIdentity>> {
        crate::recovery_recipient::read_recipient_identity(&self.conn, &target_vault_id)
    }

    /// **MVP-4-L L-0a-2.2 recoverer role.** Peek (decrypt-then-read) the
    /// recipient X25519 secret for `target_vault_id`. Used per-share-ingest
    /// — the secret remains persisted for subsequent ingest calls in the
    /// same attempt; call [`Self::clear_recovery_recipient`] on
    /// finalize / cancel to wipe.
    ///
    /// The returned secret bytes come wrapped in `Zeroizing` — callers
    /// MUST NOT hold them past their immediate use.
    ///
    /// Session-gated (Active — needs the VDK AEAD key to decrypt).
    ///
    /// # Errors
    ///
    /// [`StoreError::NotUnlocked`] if no Active session;
    /// [`StoreError::AuthenticationFailed`] on AEAD open failure;
    /// [`StoreError::Sqlite`] / [`StoreError::Corrupted`] on a malformed
    /// row.
    #[allow(clippy::type_complexity)]
    pub fn peek_recovery_recipient_secret(
        &self,
        target_vault_id: [u8; pangolin_crypto::keys::VAULT_ID_LEN],
    ) -> Result<Option<(u64, zeroize::Zeroizing<[u8; 32]>, [u8; 32])>> {
        let active = self.require_active()?;
        let vdk_aead = active.vdk.aead_key();
        crate::recovery_recipient::peek_recipient_secret(&self.conn, vdk_aead, &target_vault_id)
    }

    /// **MVP-4-L L-0a-2.2 recoverer role.** Clear (DELETE) the recovery
    /// recipient row for `target_vault_id` — atomically wipes the at-rest
    /// ciphertext of the ephemeral secret. Called on
    /// `vault_finalize_recovery` / `vault_cancel_recovery` so the secret
    /// never lingers past attempt closure (Decision A).
    ///
    /// Returns `true` if a row was deleted, `false` if none existed.
    ///
    /// Loaded-only (no VDK access required for a DELETE).
    ///
    /// # Errors
    ///
    /// [`StoreError::Sqlite`] on a DB error.
    pub fn clear_recovery_recipient(
        &mut self,
        target_vault_id: [u8; pangolin_crypto::keys::VAULT_ID_LEN],
    ) -> Result<bool> {
        let tx = self.conn.unchecked_transaction()?;
        let affected = crate::recovery_recipient::clear_recipient_tx(&tx, &target_vault_id)?;
        tx.commit()?;
        Ok(affected > 0)
    }
