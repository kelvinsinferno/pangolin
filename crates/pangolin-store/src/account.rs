...
/// Re-export of the TOTP parameter set ([`pangolin_totp::TotpParams`]) —
/// the V2 [`AccountIdentity`] TOTP slot stores `{ totp_secret bytes,
/// totp_params }`. V0/V1 accounts read with [`TotpParams::default`]
/// (SHA-1 / 6 digits / 30 s).
pub use pangolin_totp::{TotpAlgorithm, TotpParams};
...
/// V2 payload-version discriminator (MVP-1 issue 1.7).
///
/// The V1 8-key map with the `totp_secret` byte-string key replaced by a
/// nested `totp` map carrying `{ secret, algorithm, digits, period }`.
/// Arity stays 8; V1↔V2 are distinguished by the `payload_version`
/// integer (read before the `totp[_secret]` key in canonical key order),
/// so an older build reading a V2 body sees `payload_version = 2 > MAX`
/// and surfaces the §18.7 "requires upgrade" status. See
/// [`PAYLOAD_VERSION_V0`].
...
pub struct AccountIdentity {
    ...
    /// Crate-private; reach via
    /// [`crate::vault::Vault::reveal_totp_secret`]. The RFC 6238 code is
    /// generated by `pangolin-totp` from these bytes + [`Self::totp_params`].
    pub(crate) totp_secret: SecretBytes,
    /// TOTP parameters (algorithm / digits / period). V0/V1 accounts
    /// hydrate to [`TotpParams::default`] (SHA-1 / 6 / 30); V2 stores the
    /// configured values. Ignored when `totp_secret` is empty.
    pub(crate) totp_params: TotpParams,
}

impl AccountIdentity {
    /// Construct an [`AccountIdentity`] from already-allocated
    /// `SecretBytes` field owners with the RFC-default TOTP params.
    /// **No validation runs here**; callers must invoke
    /// [`AccountIdentityDraft`] or pass through the validating path.
    #[must_use]
    pub fn new_unchecked(
        ...
        Self::new_unchecked_with_totp_params(
            display_name,
            tags,
            notes,
            urls,
    ...
    /// Like [`Self::new_unchecked`] but with explicit TOTP params (the
    /// V2 read path / `account_update` set path). **No validation.**
    #[must_use]
    #[allow(clippy::too_many_arguments)]
    pub fn new_unchecked_with_totp_params(
        display_name: SecretBytes,
        tags: Vec<SecretBytes>,
        notes: SecretBytes,
        urls: Vec<SecretBytes>,
    ...
    /// The TOTP parameters for this account.
    #[must_use]
    pub fn totp_params(&self) -> TotpParams {
        self.totp_params
    }
    ...
    pub fn ct_eq(&self, other: &Self) -> subtle::Choice {
        ...
        // TOTP secret + params.
        acc &= self.totp_secret.expose().ct_eq(other.totp_secret.expose());
        acc &= subtle::Choice::from(u8::from(self.totp_params == other.totp_params));
        acc
    }
}
...
pub mod validate {
    ...
    ///
    /// Any non-empty length up to [`limits::TOTP_SECRET_MAX_BYTES`] is
    /// accepted (we do not enforce the RFC 4226 ≥ 128-bit recommendation
    /// — real-world secrets are frequently shorter, and generation is
    /// well-defined for any non-empty key). An empty secret means "no
    /// TOTP configured".
    ...
    /// Validate the TOTP parameter ranges (algorithm always valid;
    /// `digits ∈ {6,7,8}`; `period ∈ 1..=3600`). Maps a
    /// [`pangolin_totp::TotpError`] to a [`StoreError::Validation`] with
    /// `kind = "totp_params"`.
    pub fn totp_params(params: &super::TotpParams) -> Result<()> {
        params.validate().map_err(|e| StoreError::Validation {
...
pub struct AccountIdentityDraft {
    ...
    /// TOTP parameters. Ignored when `totp_secret` is empty; defaults to
    /// SHA-1 / 6 / 30 (the RFC 6238 baseline).
...
impl AccountIdentityDraft {
    ...
    pub fn validate_into_identity(
        ...
        if !self.totp_secret.expose().is_empty() {
            validate::totp_params(&self.totp_params)?;
        }
        ...
        let totp_params = if self.totp_secret.expose().is_empty() {
            TotpParams::default()
        } else {
            self.totp_params
        };

        Ok(AccountIdentity::new_unchecked_with_totp_params(
            display_name_bytes,
            tag_bytes,
            notes_bytes,
            url_bytes,
...
pub struct AccountIdentityPatch {
    ...
    /// set. `None` here when leaving TOTP unchanged or clearing it; when
    /// setting a secret without supplying params, `Some(TotpParams::default())`.
...
impl AccountIdentityPatch {
    ...
    pub fn apply(
        ...
        if let Some(p) = self.totp_params.as_ref() {
            validate::totp_params(p)?;
        }
        ...
        match self.totp_secret {
            Some(Some(bytes)) => {
                current.totp_secret = bytes;
                current.totp_params = self.totp_params.unwrap_or_default();
            }
            Some(None) => {
                current.totp_secret = SecretBytes::new(Vec::new());
                current.totp_params = TotpParams::default();
            }
            None => {
                if let Some(p) = self.totp_params {
                    // Params-only update (keep the existing secret).
                    current.totp_params = p;
                }
            }
        }
