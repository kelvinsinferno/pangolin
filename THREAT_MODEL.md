...
3. **Adversary forges a signed revision under a stolen secp256k1 scalar.** Defense: out of scope per the top-level "Cryptographic failures" boundary (same as 3.2 #3); secp256k1 ECDSA is assumed sound. 3.1 inherits the 3.2 L5 session-gate ("signing requires unlock") via `Vault::sign_revision_v1` calling `require_active()` before threading the wallet into the builder; an attacker who has the secp256k1 scalar has already breached 3.2's lifecycle invariants. The `sign_revision_v1_requires_active_session` test pins the three teardown paths (Locked / idle-expired / absolute-expired).
