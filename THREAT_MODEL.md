...
1. **L-rpc-spoof-events — malicious RPC (HTTP **or** WS) returns a `RevisionPublished` event with a forged signature for a vault the user owns.** Defense (L5): per-event `recover_signer_v1_raw` verifier runs on every decoded event; rejects with `SignerRecoveryFailed` if the signature does not curve-recover. Plus L4 — the `eth_getLogs` filter (HTTP) / `eth_subscribe("logs", filter)` (WS) pins contract address + event signature hash; plus L3 — `eth_chainId` cross-check at HTTP provider construction (the WS provider runs the L4 contract-address pin at every recv via the shared `verify_alloy_log` helper). Note: the deployed `RevisionPublished` event surface does NOT carry the signature bytes (only the recovered `signer` field), so the load-bearing defense in the current contract reduces to L3 + L4 + the contract's own `ecrecover` at publish time. **Issue #99 (2026-05-18) — applies equally to HTTP + WS:** the WS recv loop in `Vault::sync_from_chain` calls the SAME `verify_alloy_log` helper as the HTTP `fetch_chunk` path (L2 byte-identical verification), so the trust posture is preserved across both transports. The verifier helpers + the synthetic-signed-event test path (`verify_signed_event`) are wired end-to-end so a future v1.1 event that re-emits the signature flips the check on without code changes. Tests: `recover_signer_v1_tampered_signature_diverges`, `recover_signer_v1_raw_rejects_high_s`, `recover_signer_v1_raw_rejects_invalid_v_byte`, `verify_signed_event_detects_signer_field_mismatch`, `hermetic_ws_event_passes_verification_with_same_defenses_as_http`, `hermetic_ws_malicious_foreign_address_event_rejected`. Adversary leverage: mitigated to "RPC returns zero events" (the L-rpc-omits-events row below).
...
9. **L-ws-silent-disconnect (issue #99) — the WS server drops the TCP socket silently (RST without close frame). The recv loop would block forever; the user thinks sync is happening; no events arrive.** Defense: alloy's `WsConnect` sends WS-level ping/pong every `WS_KEEPALIVE_INTERVAL_SECS = 30` seconds (RFC 6455); a missing pong inside the next keepalive window flags the connection errored + triggers alloy's auto-reconnect. If alloy exhausts its built-in retries (default 10 × 3s base interval), the subscription's broadcast channel closes; `recv_next_event` returns `SubscriptionClosed`; the orchestrator increments `report.ws_drops` + backs off via `next_reconnect_backoff_ms` up to `WS_CIRCUIT_BREAKER_THRESHOLD = 5` consecutive failures, then falls through to HTTP polling for the rest of the session. L10 invariant: WS open-fail / mid-session-drop NEVER fails the sync — the orchestrator always reaches `Ok(SyncReport)` with `event_source: HttpPolling` set honestly. Tests: `hermetic_ws_recv_loop_detects_silent_socket_close_within_keepalive_window`, `hermetic_ws_circuit_breaker_degrades_to_http_after_5_consecutive_open_failures`. Adversary leverage: at worst, a slow sync after a silent disconnect (the user sees `event_source: HttpPolling` in the `SyncReport`). **L-ws-alloy-pubsub-masks-fast-drops caveat (below)** narrows this defense: the orchestrator's breaker only trips on the SLOW failure mode (alloy gives up reconnecting). The FAST accept-then-drop storm is absorbed silently by alloy and not visible at the orchestrator's `recv_next_event` layer.

10. **L-ws-alloy-pubsub-masks-fast-drops (issue #99 F-4 re-audit empirical finding) — alloy 2.0.4's `alloy-pubsub` layer transparently reconnects on accept-then-drop WS storms; the orchestrator's circuit-breaker NEVER trips on this failure mode because no `SubscriptionClosed` surfaces at `recv_next_event`. A malicious or buggy RPC that accepts WS connections + replies to `eth_subscribe` + immediately drops can spin alloy's pubsub layer indefinitely (~180 reconnects/sec measured against the hermetic mock) without the user falling back to HTTP. The user's sync APPEARS to succeed (`Ok(SyncReport)` returned, no error) but no events land — a DoS-style failure mode masked as "everything's fine."** Defense (partial, in the current sync path): the HTTP Stage 1 backfill runs on every `sync_from_chain` call BEFORE the WS Stage 2 tip-follow opens, and the L3 chain-id pin on the HTTP provider HARD-aborts if the RPC is on the wrong chain. So an adversarial RPC can't silently swap chains, and the user's catch-up cadence degrades from "real-time WS push" to "next pull-cycle HTTP poll" (~60s) without losing events — but the storm CAN drive `event_source: WebSocket` to never set (`ws_path_used` stays false), so the host UI's "Synced via WebSocket" telemetry quietly becomes "Synced via HTTP polling" and the user has no signal that something is wrong with the WS endpoint. **Root cause analyzed:** `alloy-pubsub-2.0.4/src/service.rs::reconnect_with_retries` increments its retry counter ONLY on FAILED reconnects; every accept-then-drop cycle "succeeds" at WS-handshake + eth_subscribe (the server's reply arrives before the close), so `max_retries` never trips. `WsConnect::with_max_retries(0)` does NOT help — the underlying counter still observes "success" on each cycle. **Architectural follow-up DEFERRED from #99:** a direct WS transport (`tokio-tungstenite` + hand-written JSON-RPC framing — same shape the WS hermetic mock uses) would surface every drop in real time at the orchestrator's recv layer. Weighed against a heuristic timing-based wrapper (vulnerable to adversarial threading of the keepalive signal) and an alloy fork (maintenance + drift burden); the direct-transport path is the project's hand-roll-security-critical pattern (KDBX, TOTP, encrypted-export, ciborium-ll handshake) applied to the WS surface. Tracked in the issue backlog. Tests: `hermetic_ws_accept_then_drop_subscribe_mock_mode_shape_pin` (renamed from `..._counts_toward_breaker` to drop the orchestrator-level claim the test didn't actually pin; see env-quirk #14 hermetic-only-testing trap). Adversary leverage: bounded by HTTP backfill running every pull cycle — events still land via HTTP — but real-time tip-follow degrades silently to ~60s polling cadence; the user's WS endpoint may be hostile without their knowledge.
11. **L-ws-reconnect-storm (issue #99) — RPC hits a rate-limit or TCP-level flood limiter; client retries every 250ms; gets IP-banned.** Defense: exponential backoff in `next_reconnect_backoff_ms` (250ms → 30s, doubling, cap at `WS_RECONNECT_MAX_BACKOFF_MS = 30_000`); circuit breaker at `WS_CIRCUIT_BREAKER_THRESHOLD = 5` consecutive failures (Q-b Option β) trips into HTTP polling for the rest of the session; the counter resets on the next `sync_from_chain` invocation so a transient ban doesn't permanently block. Tests: `hermetic_ws_backoff_doubles_on_consecutive_open_failures`, `hermetic_ws_reconnect_storm_caps_at_30s_then_circuit_breaks`. Adversary leverage: bounded — a malicious RPC can degrade WS to HTTP polling for the session but cannot CPU-spin the client. **Post-handshake rate-limit variant caveat:** the breaker defense above fires on the TCP-refuse / open-fail subset of storms (`WsOpenError::ConnectFailed`). A closely-related variant where the server accepts the WS upgrade, replies to `eth_subscribe`, then closes due to rate limit falls under L-ws-alloy-pubsub-masks-fast-drops (row 10 above) and bypasses this breaker — alloy absorbs that storm silently below the orchestrator's recv layer.
12. **L-ws-event-replay (issue #99) — the WS connection reconnects and the server re-emits events the client already saw.** Defense (L7 from #99 plan-gate = L12 from 4.1): `Vault::ingest_pending_chain_revision` is idempotent — canonical-hash + chain-anchor match returns `IngestOutcome::AlreadyPresent` as a storage-layer no-op. No new dedupe layer at the WS path; the storage-layer idempotency is the load-bearing defence. Test: `hermetic_ws_duplicate_event_across_reconnect_is_noop_at_verify_layer` (verifier is stateless + produces identical `VerifiedRevisionEvent` shapes for the same log on second presentation; storage idempotency covered by `ingest_chain_revision_idempotent`). Adversary leverage: none — duplicate events are observable as storage no-ops.
13. **L-ws-out-of-order (issue #99) — the WS server pushes events out of block order (server-side queuing or NAT-routing artefact).** Defense: block-order is NOT load-bearing — each event carries `(block_number, log_index, sequence)`; `ingest_pending_chain_revision` keys on `(vault_id, sequence)` not insert order; `revisions_for` returns rows ordered by `sequence` regardless of insert order. The reorg detector uses a `BTreeMap` (sorted on read). Test: `hermetic_ws_out_of_order_events_ingest_in_canonical_order` (wire order != storage order; the verifier preserves each log's anchor + the storage layer handles canonical ordering on read). Adversary leverage: none.
14. **L-ws-tls-downgrade (issue #99) — the WS RPC server advertises only `ws://` (cleartext); the requested `vault_id` topic + subscription metadata leak on-wire to a network observer.** Defense: `check_ws_scheme(ws_url, env)` rejects `ws://` for production envs (`BaseSepolia`, future `BaseMainnet`) with `WsOpenError::UnsupportedScheme` BEFORE any network I/O. The `Dev` env permits `ws://` (hermetic tests against the local mock + anvil). The deployment-pin test `deployment_json_pins_match_rust_constants::deployment_json_ws_default_uses_wss_scheme_for_base_sepolia` enforces `chain.ws_default` starts with `wss://` so the source-of-truth pin is also TLS. Tests: `hermetic_ws_rejects_ws_scheme_for_base_sepolia`, `hermetic_ws_tls_downgrade_blocked_for_every_production_env`, `check_ws_scheme_rejects_cleartext_for_base_sepolia`. Adversary leverage: none in production (`ws://` refuses to open); in dev, `ws://` is intentional for the mock server harness.
15. **L-ws-feature-leak-pulls-ring (issue #99) — alloy's `provider-ws` feature transitively selects `ring` (the banned TLS crypto provider) instead of `aws-lc-rs`; `deny.toml`'s `ring` ban fires + the build fails.** Defense: alloy 2.0.4's `provider-ws` feature explicitly selects `transport-ws` which uses `aws-lc-rs`. Verified post-flip via `cargo tree -i ring` returning zero rows. The CI gate `scripts/check-no-ring.sh` (wired into the `invariants` job in `.github/workflows/ci.yml`) catches any future drift at PR time. `deny.toml`'s `ring` ban is the secondary defence. Tests: the CI script itself (asserts `cargo tree -i ring` returns 0 rows) + the workspace-wide build (`cargo deny check`). Adversary leverage: structural — if the script ever fires it's a build-blocker, not a runtime issue.
...
### anvil-fork CI harness (issue #101) (`scripts/anvil-ci.sh` + `.github/workflows/ci.yml` `anvil-integration` job + `crates/pangolin-chain/src/test_env.rs` + scoped `chain_id` threading in `secp256k1_signing` + `chain_submit`)

> Source: `docs/issue-plans/101-anvil-fork-ci.md` + `DECISIONS.md` #101 (R-a..R-e) + the #101 amendment. See `docs/architecture/anvil-ci-harness.md` for the full design.

**env-quirk #14 row UPDATE (the THIRD defense layer — the one that runs on EVERY PR without secrets).** Pre-#101 the two #98 layers were: (1) bytes-parsing surface via hermetic replay fixtures (every PR), and (2) contract-execution surface via `#[ignore]` live tests against Base Sepolia (pre-release, requires secrets/RPC). The gap: the **contract-execution surface** — the exact place the 3.3 `keccak(encPayload)`-vs-preimage bug lived — was only exercised pre-release by a human, never on a PR. #101 closes that:

- **Contract-execution surface (every PR via the `anvil-integration` job).** A local `anvil` node runs the ACTUAL deployed bytecode; `scripts/anvil-ci.sh all` deploys RevisionLogV1 + EntitlementRegistry, generates `dev.json`, funds the deterministic test wallet, and runs `publish_v1_live_d017_smoke` + `live_pull_once_against_d017_advances_checkpoint` + `live_balance_query_against_d017_wallet` in dev mode. The publish test's signature is `_recover`ed by the REAL contract — a future calldata-construction drift (or an alloy ABI-codec change) that would revert on a live publish turns this job RED at PR time, with NO secret RPC URL and NO funded mainnet/testnet wallet (R-d: fresh anvil, deterministic, hermetic-with-respect-to-the-network).

1. **L-harness-skips-instead-of-failing — a missing `dev.json` / RPC / env var in dev mode causes the in-scope test to skip-clean (the 3.3 bug "passed" precisely because the live test skipped); the harness then reports green while testing nothing.** Defense (load-bearing): **L6** — in dev mode (`PANGOLIN_CHAIN_ENV=dev`) the pre-#101 skip-clean `return` becomes a HARD error. `test_env::require_or_fail` panics in dev mode (clean-skips only in base-sepolia mode); the balance test `assert!`s on `BASE_SEPOLIA_DEV_WALLET`. A misconfigured harness turns CI RED, never green-while-empty. Adversary leverage: none — designed-in fail-closed; verified by the L9 regression-proof (a broken publish calldata turned the test RED, not skip).

2. **L-anvil-flake-masks-real-failure — a racy fixed-sleep "wait for anvil" lets tests run before the node is ready (or after it died), producing a flaky pass/fail that erodes trust in the gate.** Defense (load-bearing): **L5** — `scripts/anvil-ci.sh` POLLS `cast block-number` for readiness (bounded max-attempts, fail-closed; NO fixed sleep) and checks the anvil PID is alive each poll; a `trap` tears anvil down on any exit (success or failure). Deploy / artefact-parse failures are fail-closed (`exit 1` with the captured log). Adversary leverage: none — determinism is the property that makes the gate worth gating merges on.

3. **L-prod-signs-with-untrusted-rpc-chainid — the scoped signing change reads `eth_chainId` to resolve the EIP-712 + envelope chain id; a malicious RPC could report a foreign chain id and steer a production (BaseSepolia) signature to bind the wrong domain (cross-chain replay surface).** Defense (load-bearing): production NEVER sources its signing chain id from the RPC. For a fixed env (`BaseSepolia` → `84_532`), `resolve_envelope_chain_id` reads `eth_chainId` ONLY to cross-check it against the pinned value (the pre-existing L-rpc-spoof guard) and returns the PINNED id — a lying RPC is rejected with `ChainIdMismatch`, it can never reach the signer. ONLY the `Dev` / local-anvil path (`env.chain_id() == None`, by construction a fresh local node, never a fork of a public chain — R-d) reads the live id. `Dev.chain_id()` STAYS `None` (the enum + `chain_env_chain_ids_are_pinned` test are untouched — L2). Adversary leverage: none for production — the pinned-`84_532` path is byte-identical to pre-#101 (the hermetic `DOMAIN_SEPARATOR_BASE_SEPOLIA_V1` / `REVISION_TYPEHASH_V1` / 3.6-baseline-signature pins stay green); the only RPC-sourced id is on a trusted local dev chain.

4. **L-stale-dev-json-fixture — a committed `dev.json` rots silently the moment a deploy script / constructor / order changes; tests then call a dead/wrong address while appearing to pass.** Defense (load-bearing): **R-e** — `dev.json` is GENERATED at CI runtime from the actual fresh deploy output (parsed from the structured broadcast artefact `run-latest.json`, true-by-construction per run) and is `.gitignore`d so it can never be committed. Adversary leverage: none — there is no stored fixture to rot.

**Invariant additions:** L1 (production-code change LIMITED to the scoped signing chain-id threading) + L2 (BaseSepolia pins byte-identical; `Dev.chain_id()` enum+test untouched) + L3 (existing CI jobs untouched; the job is additive) + L4 (Foundry pinned `v1.0.0`, same as the contracts jobs) + L5 (deterministic poll-ready/trap/fail-closed) + L6 (skip→hard-error in dev mode) + L7 (no new `=`-pinned external Rust dep — anvil/forge/cast are binaries) + L8 (`forbid(unsafe_code)` + AGPL SPDX on the new script + seam) + L9 (the regression-proof: a broken publish calldata MUST turn the publish test RED against anvil — verified at build time, then reverted). HIGH-1 zero-serde in pangolin-crypto preserved (no new serde-shaped dep). The signing change is scoped so the recover path (`recover_signer_v1_raw` via `chain_sync::verify_signer_or_reject`, which is sync + providerless) resolves `env.chain_id().unwrap_or(0)` internally — byte-identical to pre-#101 — rather than threading the live id through the whole sync stack; the fresh-anvil pull test never reaches recovery (its test vault has no on-chain events).
