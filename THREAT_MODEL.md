...
7. **KDF parameter tampering on disk.** Defense: KDF params live in plaintext on the meta row (they MUST be readable before unlock to feed the same Argon2id parameters back into `derive_seed`). Sub-floor params are rejected by `KdfParams::validate` at `pangolin-crypto`'s public boundary, surfacing as `StoreError::KdfRejected`. Tampering that keeps params in-range but changes their values (e.g., shifting `time_cost` from 3 to 4, or flipping a bit in the salt) produces a different derived seed; the resulting authority cannot unwrap the stored `WrappedVdk` and the unlock path returns `StoreError::AuthenticationFailed` — indistinguishable from a wrong password. Verified by the `adversarial_kdf_param_tampering_fails` integration test.
