...
| Ephemeral local indexer | MVP-2 | DOCUMENTED (issue 4.2 skeleton + 4.3 AEAD + zero-fill + ephemeral key) |
...
### Ephemeral local indexer (4.2 skeleton + 4.3 AEAD + zero-fill + ephemeral key) (`pangolin-indexer::{session, protocol, cipher, error}` + `pangolin-indexer` binary)
...
1. **L-temp-file-leak — a crashed indexer (panic / SIGKILL / OOM / power-loss) leaves a temp file on disk containing chain-event data.** Defense post-4.3 (at-rest case substantially closed; runtime case has a deferred gap — see "Deferred / known gap" below): (a) `tempfile::NamedTempFile` gives the random path (L1 — `O_CREAT | O_EXCL | O_NOFOLLOW` posture); (b) the AeadCipher (`XChaCha20-Poly1305` from `pangolin-crypto::aead`, per-page random 24-byte nonce, sealed under an ephemeral 32-byte key derived via HKDF-SHA256(device_seed; salt = per-run 16-byte nonce; info = `"pangolin-indexer-tempdb-key-v1"`) — see `pangolin_chain::derive_indexer_key`) is constructed at session start, probed end-to-end, and ready to wrap BLOB columns; (c) on session Drop the `secure_zero_fill` helper (4.3 R-c) does two passes — random 4-KiB chunks + fsync, then zeros + fsync — before the NamedTempFile Drop unlinks; (d) the Drop's load-bearing ordering closes the SQLite Connection FIRST (via `Option::take`) so the `secure_zero_fill` re-open succeeds on Windows; (e) `panic = unwind` makes the Drop fire during stack unwinding (L11 panic branch — verified via `cleanup_on_panic_unwinds_temp_file`); (f) OS-temp-dir GC is the SIGKILL / `panic = abort` fallback (the SQL path's plaintext is at-risk only during runtime; the zero-fill + ephemeral key go with the process). **Deferred / known gap:** the `persist_chunk` INSERT and `handle_pull` SELECT paths still operate on PLAINTEXT BLOBs during the indexer's runtime — the AeadCipher is wired in for the trait + constructor probe + zero-fill discipline but NOT YET wrapped around individual column INSERTs/SELECTs. **A concurrent disk-access attacker observing the temp DB file mid-run can read plaintext chain-event data.** The threat model accepts this because: (i) the runtime window is short (5-min idle default; 1-hour absolute ceiling); (ii) the file lives in `env::temp_dir()` (OS-managed; not a user-visible directory); (iii) the attacker class with mid-run filesystem access typically also has memory access (`/proc/<pid>/mem` on Linux, task ports on macOS) which would expose the AEAD key itself, defeating an encrypted-runtime scheme anyway. **Per-column wrapping + AAD binding (`vault_id, page_id, schema_version`) both land together as an additive follow-up** — neither has security value without the other, and the wire-format stays compatible so they ship without migration. **Documented limit:** SSD wear-leveling may write the overwrite to a different physical block than the original data; the original block persists until TRIM/garbage collection. Adversary leverage post-4.3: runtime-plaintext window during the session (mitigated by short process life + OS-temp-dir conventions + memory-access threshold) + bounded ciphertext-residue post-Drop (mitigated by zero-fill + key zeroized at process exit). Tests post-4.3: `secure_zero_fill_overwrites_file_content`, `secure_zero_fill_empty_file_is_noop`, `secure_zero_fill_handles_partial_chunk`, `secure_zero_fill_handles_multi_chunk`, `dropping_session_does_not_panic_with_aead_cipher`, plus all 4.2 lifecycle tests (`cleanup_on_panic_unwinds_temp_file`, `cleanup_on_drop_in_cancelled_task`, `cleanup_when_multiple_sessions_dropped`, `cleanup_survives_idle_timeout_path`, `session_lifecycle_normal_exit_deletes_temp_db`).
