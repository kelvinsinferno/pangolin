...
2. **Attacker recovers the secp256k1 scalar from a leaked memory dump of an `Active` session.** Defense: **out of scope** per `THREAT_MODEL.md`'s top-level "Out of scope: fully compromised operating systems / Kernel-level malware". 3.2 inherits the 1.5 in-memory `DeviceKey` boundary verbatim — the secp256k1 scalar lives inside the wallet's `k256::SecretKey` whose `Drop` zeroizes (L-zeroize); a memory-dumping attacker has the same access to the Ed25519 seed by the same path. 3.2 does NOT widen this surface. The `no_evm_secret_after_drop` regression test in `crates/pangolin-chain/src/evm.rs::tests` pins the determinism contract end-to-end so a future refactor that introduces a static / `OnceCell` / cross-session signer cache (which would defeat the drop-with-session discipline) is caught.
