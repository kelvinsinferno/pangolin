...
## Issue #106 — MULTI-DEVICE (one vault, several devices) — 2026-05-21

**Why (Kelvin):** "there is no point to saving on chain unless it's a multi-device system." The on-chain RevisionLog exists to SYNC a vault across devices; single-device on-chain (the prior state) is pointless. Multi-device is a whitepaper-promised capability (VDK fans out per device key; "devices revoked without recovery"). Now the foundational priority. Full architecture + per-sub-issue decisions in `docs/issue-plans/{106,106a,106b,106b2}-*.md` §0a (LOCKED). #103-C revocation-on-read PARKED — folds into #106d (its predicate generalizes from "single current authority" to "current authorized device SET").

**Architecture (LOCKED §0a of `106-multi-device.md`):** per-device keys (each device its own enclave key + biometric unlock; ONE master password; VDK wrapped per-device). VDK handoff = sealed-box pairing (reuse #104a crypto_box). Trust: primary authorizes + survivor-promotion + preserve-survivors-on-loss; full guardian recovery only for lost-everything. Device removal = ON-CHAIN authorized set (addDevice+removeDevice, vault-authority-gated; self-sovereign, not the forbidden admin). Honor = signer in current on-chain set. Recovery preserves surviving devices.

**Split (build order): #106a → #106b-1 → #106b-2 → #106c → #106d → #106e.**
- **#106a RevisionLogV2 contract — MERGED `6e5bf74`.** On-chain authorized-device SET; publishRevision gated on set-membership; addDevice/removeDevice (manager-signed EIP-712 + per-vault nonce); 48h survivor-promotion (propose/finalize/cancel); Option B V2-local deviceManager live-read-aligned to RecoveryV1.vaultAuthority (closes #103-C GAP 2); bootstrapVault; MAX_DEVICES=32; EIP-712 domain v"2". Cardinal rules honored. Adversarial audit CLEAN; forge invariants 10k×32.
- **#106b device-pairing crypto (CRUX = R1: device-revoke RE-CREATES the VDK for forward secrecy; "re-wrapped not re-created" is the RECOVERY invariant, separate op).** **#106b-1 pairing-seal + per-device-wrap PRIMITIVE — MERGED (`73fd1c1`; empty-test hotfix `98a35b8`).** `pangolin-crypto::pairing` — derive_x25519_pairing_key + seal_vdk_to_device/open + wrap_vdk_for_device/unwrap; distinct HKDF infos; audit CLEAN. **#106b-2 VDK-rotation-on-revoke FLOW — MERGED `9f67221`.** rotate_vdk_for_survivors (fresh VDK epoch sealed to survivors + Q-d=(a) re-split RWK' to guardians) + atomic Vault::commit_vdk_rotation (prompt-on-revoke password anchor; single-tx; device-key re-seal under new VDK) + vdk_chain epoch model (on-chain encPayload immutable → old entries stay under old VDK, removed device denied new epoch). Audit CLEAN (forward secrecy, escrow re-point, atomicity discrimination proven RED).
- **#106c (client device-add flow + DeviceRemoved→rotation trigger), #106d (revocation generalization, folds #103-C), #106e (pairing UX) — NOT yet built.**
...
- **#106d — V2 revocation honor gate + retroactive re-eval — MERGED `a622bba`.** Wired the V2 honor gate into both sync arms (honor iff recovered signer ∈ live on-chain authorizedDevice set; non-set counted+not-ingested; `read_authorized_set_v2` FAIL-CLOSED on any read error) + retroactive re-eval (re-check stored V2 rows, mark-revoked both-directions, filter revoked from head/history/content reads). Keys on the **recovered signer** (persisted at ingest via the additive `revisions.recovered_signer` column), NOT the opaque `device_id` — the fix-pass closed an under-revocation vector the audit found (an in-set device stamping `deviceId=leftpad(other)` would survive its own removal); NULL→conservatively revoked. V1 path untouched/permissive (Q-e). No contract change. Adversarial audit + fix-pass re-audit CLEAN; full workspace `--lib --tests` green on MSVC + anvil `revocation_honor_gate_remove_then_read` E2E green.
- **Parked #103-C branch CLOSED (Q-d):** `worktree-agent-a3f6272eca2f476be` is superseded — its store plumbing (revoked column/migration/read-filters/retroactive pass) was salvaged into #106d with the predicate re-keyed to set-membership; its lineage inference (`revocation.rs`/`AuthorityLineage`/`read_authority_lineage_v1`/`RecoveryFinalized` fold) is DISCARDED (the live on-chain set is the truth). The branch is not to be merged; treat as abandoned.
...
### #106e-0b — production guardian-escrow ONBOARDING — MERGED 2026-05-22

The missing recovery prerequisite split out of the #106e-1 plan-gate: until a vault can ONBOARD guardians there is no escrow for `complete_rotation`/`recover_from_shares` to act on (the only initial-escrow writer was the test-only `__test_onboard_recovery_escrow`). Plan `docs/issue-plans/106e0b-guardian-onboarding.md` §0a.

- **Q-a → Option B: ONE shared onboard primitive.** Lifted the split-and-seal composition into `pangolin_crypto::escrow::onboard_escrow` (upstream of both store + core); the new store `Vault::onboard_guardians` calls it directly, and `pangolin_core::recovery::onboard_guardian_escrow` (the rotation/recovery RE-split) was refactored to a byte-PRESERVING wrapper over the SAME fn — so the initial-onboard and the re-split can never drift. NO dep-arrow problem (onboarding needs only `pangolin-crypto`, unlike #106e-0's rotation/recovery).
- **Q-b → re-onboard REPLACES the prior generation** (`write_recovery_escrow_tx` DELETEs prior guardian rows). **Q-c → first onboard at GENESIS epoch (0)**; rotation/recovery bump it.
- **Build note:** the builder agent STALLED mid-self-verification (watchdog) with the code written; the main session finished it on the host (fixed a real L5-test bug — recovering into a blank vault vs. in place — + two clippy lints + rustfmt).
- **Audit CLEAN (no findings)** — byte-preservation confirmed field-by-field (epoch encoding, share ordering, WrapContext, RWK/wrap/split all identical); secret hygiene, single-tx atomicity, replace-semantics, bounds all pass. Host gates: `cargo test --workspace` green (incl. the extraction-sensitive orchestration/rotation proptests + the meta-test); `clippy -D warnings` + `cargo fmt --check` clean; recovery-escrow + device-rotate + complete-rotation anvil E2Es green on live bytecode. The L5 hermetic round-trip proves a production-onboarded escrow recovers to the byte-identical VDK.
- **Process miss (recorded):** the #106e-0 merge initially reddened main on `rustfmt` (the pre-merge host gate ran tests/clippy/anvil but not `cargo fmt --check`); fixed by `d261843`. The gate now includes `cargo fmt --check`.

### #106e-1 — recovery + rotation FFI entry points — MERGED 2026-05-22

The thin uniffi layer over the merged #106e-0 composition + #106e-0b onboarding, so host apps can drive multi-device recovery/rotation. SUBSUMES the long-pending #105b recovery-FFI. Plan `docs/issue-plans/106e1-ffi-entry-points.md` §0a.

- **Q-a → SPLIT: onboarding was its own #106e-0b** (merged first); #106e-1 = the FFI layer.
- **Q-b → the FFI READS the live authorized set itself** (engine is the single source of truth). `vault_complete_rotation` is async (`block_on_local` + the fail-closed `read_authorized_set_v2`), taking only the chain config (rpc_url/deployment_path) — the host CANNOT pass a forged/stale set. Active-session pre-check BEFORE the RPC (no wasted call on a locked vault).
- **Q-c → opened `Share` as a `SecretPassword`-style opaque `Arc<FfiOpenedShare>` Object** — exposes ONLY `byte_length()` via the `#[uniffi::export]` surface; `into_inner` (the only extraction back to a `Share`) is crate-private; `Debug` redacts.
...
- **Q-e → ONE slice** (5 bindings: `vault_pending_rotations`, `vault_complete_rotation`, `vault_guardian_open_share`, `vault_recover_from_shares`, `vault_onboard_guardians` — the #106e-0b FFI binding folded in).
- **Q-f → pairing/device-add FFI is OUT** → #106e-2 (the LOCKED plan: `docs/issue-plans/106e2-pairing-transport-sas.md`).
- **Build note:** builder finished (no stall); the main session fixed one binding issue (`vault_complete_rotation` was hitting the RPC on a locked vault and failing with `Chain` — added an active-session pre-check so it fails fast with `Session` before any RPC), plus a few clippy/fmt lints. Audit CLEAN (no findings; one LOW non-blocking note that the "exposes only length" test documents intent but can't mechanically catch a future `bytes()` getter — the real guarantee is source review).
- **Host gates (the FULL set this time):** `cargo fmt --check` clean; `cargo clippy -p pangolin-ffi --all-targets -- -D warnings` clean; `cargo test --workspace` green incl. the `no_empty_ignored_tests` meta-test + every new FFI test (fail-closed-on-bad-rpc, opened-share-length-only, exhaustive-error-mapping, locked-before-chain, length-validation negatives, onboard→open→recover round-trip through the FFI). `cargo tree -p pangolin-core | grep -ci uniffi` = 0 (the cardinal isolation invariant holds).
- **Remaining in #106e:** #106e-2 (pairing transport + SAS, plan LOCKED, builds on top of #106e-1).

### #106e-2 — device-pairing TRANSPORT + SAS + device-add FFI — MERGED 2026-05-22

The FINAL #106e slice — completes the multi-device epic end-to-end. Plan `docs/issue-plans/106e2-pairing-transport-sas.md` §0a.

- **Q-carrier (Q-a):** QR + copy-paste TEXT-STRING carriers (no relay/server — Rust emits BOTH a byte form for QR and a base32+checksum text string for camera-less devices; host moves them). Works on cameraless laptops via copy-paste.
- **Q-channel (Q-a):** BIDIRECTIONAL pubkey exchange (B→A then A→B), both compute the 6-digit SAS, both display, human compares — and ONLY then A broadcasts `addDevice` + seals the VDK. SAS is a MANDATORY gate before authorize+seal.
- **Q-b (SAS format):** 6-digit decimal; CANONICAL-SYMMETRIC (lex-sorted pubkeys → identical code on both devices regardless of role); SHA-256(`SAS_DOMAIN` ‖ lo-pub ‖ hi-pub ‖ nonce), mod 1e6, zero-padded. Domain `pangolin-pairing-sas-v0` distinct from all existing pairing/seal HKDF infos.
- **Q-d (no payload signature):** rejected — an attacker self-signs its own consistent triple, so a sig wouldn't anchor "talking to the real B"; seal-binding + SAS are the protection.
- **Q-e / Q-f:** one slice; codec in `pangolin-core::pairing_transport`, SAS in `pangolin-crypto::pairing`, FFI in `pangolin-ffi::pairing`.
- **Build notes:** parent finished the gate after the builder (sandboxed) wrote the code; ran multiple lint cleanups (fmt, 1 unused import, 1 underscore-binding rename, 1 redundant clone, 2 test-only `significant_drop_tightening` allows, 1 `usize→u8` truncation, module-level doc-lint allows on the two heavily-doc'd modules). The builder's `tests/anvil_pairing_e2e.rs` was deleted (it needed crate-private `lock_vault` access which integration-test crates can't see) — flagged as a 6.x follow-up.
- **Audit CLEAN** with 2 LOW non-blocking follow-ups: (1) no fault-injection test for `Vault::install_paired_vdk`'s atomicity (the code IS correctly atomic — single `unchecked_transaction()` over meta-write + device-key reseal; gap is test coverage); (2) restore the deleted anvil FFI E2E (composed full-stack add-device-through-FFI path; underlying components ARE anvil-tested via #106c's `device_add_remove_rotate_e2e`). Plus one observation (NOT a finding): the SAS binds the two pairing pubkeys + nonce but NOT signer/device_id — a MITM substituting just the signer would pass SAS but seal-binding still prevents VDK leak (impact bounded to DoS/gas-burn, NOT confidentiality). §0a Q-d deliberately chose this tradeoff.
- **Host gates:** `cargo fmt --check` clean; `cargo clippy -D warnings` CLEAN under both default + `--features integration-tests`; `cargo test --workspace` green incl. the workspace meta-test + the new pairing_transport (13) / SAS (7) / FFI-pairing (15) tests — including the load-bearing **L2 anti-MITM** (swapped pubkey ⇒ SAS differs) and **L3 canonical-symmetric** (both devices compute identical code).
- **#106 multi-device epic — COMPLETE end-to-end** (contract #106a → device-pairing crypto #106b-1 → VDK-rotation-on-revoke #106b-2 → device-add client #106c → V2 data-plane #106c2 → revocation gate #106d → composition layer #106e-0 → guardian onboarding #106e-0b → recovery/rotation FFI #106e-1 → pairing transport + device-add FFI #106e-2). Stays TESTNET-ONLY until D-011.
- **Remaining project work:** #107 (V1 read-topic bug, standalone) + the 2 LOW follow-ups above.

### #107 — fix V1 read-topic bug + filter-respecting hermetic mock — MERGED 2026-05-22

The dormant V1 bug: `RevisionLogV1.RevisionPublished` puts `sequence` at topic1 and `vaultId` at topic2, but `chain_sync/poll.rs:~196` (HTTP) + `chain_sync/ws.rs:~240` (WS) filtered vault_id at `.topic1(...)` — so V1 reads against a real RPC returned ZERO logs. Dormant because the alloy `Asserter` mock ignored topic filters + the only live V1 read test was `#[ignore]`'d. V2 path already correct. HIGH-impact, LOW-complexity.

- **Q-a → SMARTER MOCK + HERMETIC TESTS** (the durable choice). NEW `FilteringAsserter` (`tower::Service<RequestPacket>`, `#[cfg(test)]`-only) intercepts `eth_getLogs` and applies alloy's own `Filter::matches` + `matches_log_block` semantics. WS mock extended with a `respect_filter` flag that applies the subscribe-time filter symmetrically across capture, pending-buffer flush, and live-push paths. The hermetic test suite now catches THIS bug AND any future similar V1/V2/V0 filter regression at compile/PR time.
- **Q-b → BOTH HTTP + WS regressions.** 4 new tests: `fetch_chunk_filters_by_topic2_not_topic1` (HTTP main), `smart_mock_applies_topic_filter_buggy_topic1_filter_drops_all_logs` (HTTP filter-semantics shape pin), `hermetic_ws_subscription_filters_by_topic2_not_topic1` (WS main), `hermetic_ws_subscription_filter_topology_pins_topic2_for_vault_id` (WS filter-shape pin).
- **Fix:** 2 lines — `poll.rs:~205` + `ws.rs:~247` `.topic1(vault_topic)` → `.topic2(vault_topic)`, mirroring V2's existing correct pattern at `v2.rs:~182,~290`. Plus 5 stale-comment corrections (indexer/session.rs ×2, v2.rs docstrings ×2, docs/architecture/chain-sync.md ×2) per the "stale comments are the breeding ground for re-introduction" principle.
- **V0 untouched** (V0's event puts vaultId at topic1 — `.topic1(vault_id)` is correct there). **V2 code untouched** (already correct). **RevisionLogV2 device-mgmt events** (`DeviceAdded`/`Removed`/`PromotionFinalized` — vaultId IS topic1 as the sole indexed param) untouched.
- **Discrimination empirically proven** (parent ran it): reverting `poll.rs` to `.topic1` made `fetch_chunk_filters_by_topic2_not_topic1` panic; reverting `ws.rs` to `.topic1` made `hermetic_ws_subscription_filters_by_topic2_not_topic1` panic. Restored. Both tests genuinely catch the bug.
- **Audit CLEAN** on all 8 central checks (byte-identity vs V2, V0/V2 untouched, filter-mock semantics correct, WS `respect_filter` symmetric on all 3 paths, regressions discriminate, no new external crates, AGPL+forbid(unsafe), `#[cfg(test)]`-only mock). 3 LOW findings (preexisting stale comments) FIXED in the same commit.
- **Host gates:** `cargo fmt --check` clean; `clippy -D warnings` clean; `cargo test --workspace` green (78 ok-summaries, 0 failures, meta-test + 4 new regression tests run). No new runtime deps (`alloy-json-rpc`/`alloy-transport`/`tower` added under `[dev-dependencies]`, already transitively present via `alloy::providers::ws`).
- **MVP-3 active surface is now functionally complete** end-to-end. Remaining: D-011 external audit (the hard pre-mainnet gate) + the 2 LOW follow-ups from #106e-2 (`install_paired_vdk` fault-injection test; restored anvil FFI pairing E2E).
