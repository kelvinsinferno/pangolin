...
/// **Q5 (issue 1.8).** Generate a password via the one true generator
/// (`pangolin_core::pwgen`) — the strong defaults: 16 chars, all four
/// character classes, ambiguous chars excluded, unbiased CSPRNG draws.
/// The divergent local 64-char generator that predated the
/// `password_generate` FFI entry has been removed; `--generate-password`
/// now produces exactly what the library generator produces. See
/// `docs/architecture/password-generator.md`.
fn generate_password() -> SecretBytes {
    let generated = pangolin_core::pwgen::generate(&pangolin_core::pwgen::PwgenPolicy::default())
        .expect("the default policy is always valid");
    SecretBytes::new(generated.as_bytes().to_vec())
}
...
mod tests {
    ...
    /// **Q5 (issue 1.8).** `--generate-password` now routes through
    /// `pangolin_core::pwgen` — 16 ASCII chars, all four classes,
    /// no ambiguous chars.
    #[test]
    fn account_add_generate_password_uses_pangolin_crypto_rng_alphabet() {
        let pwd = generate_password();
        let bytes = pwd.expose();
        assert_eq!(bytes.len(), 16, "default-policy length is 16");
        ...
        for b in bytes {
            assert!(b.is_ascii() && !b.is_ascii_whitespace(), "byte 0x{b:02x}");
            assert!(!b"0O1lI|".contains(b), "ambiguous char 0x{b:02x} leaked");
        }
    }

    /// **Q5 (issue 1.8).** Two consecutive calls must produce
    /// distinct passwords (the CSPRNG is non-deterministic).
    #[test]
    ...
    /// returns printable ASCII bytes (the format the user reads).
    #[test]
    fn generated_password_format_preserved_after_med3_fix() {
        let pwd = generate_password();
        let bytes = pwd.expose();
        assert_eq!(bytes.len(), 16);
        // Every byte must be printable ASCII (no NUL / no newline /
        // no control char in what we hand to write_all).
        for b in bytes {
            assert!(b.is_ascii_graphic(), "byte 0x{b:02x} not printable ASCII");
