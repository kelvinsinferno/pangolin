...
## 2026-05-22 · BUG #107 — V1 chain-read topic1→topic2 fix + filter-respecting hermetic mock (MVP-3 active surface now functionally complete)

Plan `docs/issue-plans/107-v1-read-topic-fix.md` §0a (Kelvin sign-off 2026-05-22). **The bug:** `RevisionLogV1.RevisionPublished(uint256 indexed sequence, bytes32 indexed vaultId, …)` puts `sequence` at topic1 and `vaultId` at topic2, but `chain_sync/poll.rs:~196` (HTTP `eth_getLogs`) + `chain_sync/ws.rs:~240` (WS `eth_subscribe`) filtered `vault_id` at `.topic1(...)` — so V1 reads against a REAL RPC matched `sequence == vault_id` (almost always false) and returned ZERO logs, breaking sync for legacy V1 vaults. **Dormant** because (a) the alloy `Asserter` mock used by `chain_sync/tests.rs` returns queued responses WITHOUT inspecting topic filters; (b) the only live V1 read test (`publish_v1_live_d017_smoke`) is `#[ignore]`-marked + only tests *publishing*, not *reading*. **The 2-line fix:** `poll.rs:~205` + `ws.rs:~247` `.topic1(vault_topic)` → `.topic2(vault_topic)`, mirroring V2's existing correct pattern at `v2.rs:~182,~290` (byte-identical). **The durable infra (Q-a):** NEW `crates/pangolin-chain/src/chain_sync/filtering_asserter.rs` — `FilteringAsserter` is a project-local `tower::Service<RequestPacket>` impl (`#[cfg(test)]`-only, no production surface) that intercepts `eth_getLogs` requests, deserializes the `Filter` from the request params, and applies alloy's own `Filter::matches(&log.inner) && filter.matches_log_block(&log)` to each queued log — delegating to upstream semantics so there's no off-by-one or hand-rolled topic-slot risk. Also handles `eth_chainId` + `eth_blockNumber` with configurable values; falls through to a FIFO fallback queue for any other RPC method. The WS side (`tests/ws_mock_server.rs`) got a `respect_filter` bool (default false preserves existing tests) that captures the subscribe-time filter and applies it symmetrically across all three paths — capture, pending-buffer flush, and live-push — same `Filter::matches` predicate. **4 new regression tests** (`#[cfg(test)]`, NOT `#[ignore]`'d): HTTP `fetch_chunk_filters_by_topic2_not_topic1` (drives `fetch_chunk` against the smart mock with two canonical-V1 logs for different vault_ids, asserts exactly the requested one comes back) + `smart_mock_applies_topic_filter_buggy_topic1_filter_drops_all_logs` (HTTP filter-semantics shape pin: buggy `.topic1(vault_id)` → 0 logs; correct `.topic2(vault_id)` → 1 log); WS `hermetic_ws_subscription_filters_by_topic2_not_topic1` (drives `open_subscription` against the WS mock in `respect_filter=true` mode, asserts only the matching log emits) + `hermetic_ws_subscription_filter_topology_pins_topic2_for_vault_id` (directly pins `log.topics()[1] == sequence`, `log.topics()[2] == vault_id` so any future event-shape change is caught loudly). **Discrimination empirically proven by the parent:** reverting `poll.rs` `.topic2 → .topic1` made `fetch_chunk_filters_by_topic2_not_topic1` panic at the asserted event count; restored. Reverting `ws.rs` `.topic2 → .topic1` made `hermetic_ws_subscription_filters_by_topic2_not_topic1` timeout (no event arrived) → panic; restored. Both regressions genuinely catch the bug. **Comment cleanups (5):** `pangolin-indexer::session` ×2 (V1 read-path docs), `chain_sync/v2.rs` docstrings ×2 (which wrongly said "vaultId topic1" even though V2's code already correctly uses topic2 — more dangerous than V1's stale comments because V2 is the reference V1 mirrors), `docs/architecture/chain-sync.md` ×2 (architecture diagram). **What's deliberately UNTOUCHED:** V0 (`base_sepolia.rs:~516` — V0's event puts vaultId at topic1, its `.topic1(vault_id)` is CORRECT); V2 code (already correct); RevisionLogV2 device-mgmt events (`DeviceAdded/Removed/PromotionFinalized`'s vaultId IS topic1 because it's the sole indexed param). **Audit CLEAN** on all 8 central checks: byte-identity vs V2 ✓, V0/V2 untouched ✓, `FilteringAsserter` delegates to alloy's `Filter::matches` (no hand-rolled topic logic) ✓, WS `respect_filter` symmetric on all three paths ✓, regressions discriminate (verified empirically by the parent) ✓, no new external crates (dev-deps `alloy-json-rpc`/`alloy-transport`/`tower` were already transitively present via `alloy::providers::ws`) ✓, AGPL SPDX + `forbid(unsafe_code)` + `#[cfg(test)]`-gated mod ✓, stale comments corrected ✓. 3 LOW preexisting stale-comment findings the audit flagged were FIXED in the same commit (per the §3.3 principle "stale comments are the breeding ground for re-introduction"). **Gates (host, since agents are cargo-sandbox-blocked):** `cargo fmt --check` clean; `cargo clippy -p pangolin-chain --all-targets -- -D warnings` clean (after a handful of test-only allows for `items_after_statements`/`too_many_lines`/`needless_pass_by_value`/`unused_async` on test helpers); `cargo test --workspace` green (78 ok-summaries, 0 failures, the workspace `no_empty_ignored_tests` meta-test ran + all 4 new regression tests ran). **MVP-3 ACTIVE SURFACE NOW FUNCTIONALLY COMPLETE END-TO-END:** all of #102 (RecoveryV1 contract) + #103 (recovery client + lifecycle) + #104a (escrow primitive) + #104b (recovery orchestration) + #105a (atomic re-split persistence) + the full #106 multi-device epic (#106a contract → #106b-1 pairing crypto → #106b-2 VDK rotation → #106c device-add → #106c2 V2 data-plane → #106d revocation gate → #106e-0 composition layer → #106e-0b guardian onboarding → #106e-1 recovery/rotation FFI → #106e-2 pairing transport + device-add FFI) + this #107 fix are merged + CI-green on main. Stays TESTNET-ONLY until D-011 (the external audit is the hard pre-mainnet gate). **Open follow-ups:** 2 LOWs from #106e-2 (`Vault::install_paired_vdk` fault-injection test for the atomicity property; restore the anvil FFI pairing E2E that needed crate-private `lock_vault` access). No active feature work outstanding.

## 2026-05-22 · MVP-3 issue #106e-2 — device-pairing TRANSPORT (QR + copy-paste) + SAS + device-add FFI (the FINAL #106e slice; #106 multi-device epic COMPLETE end-to-end)

Plan `docs/issue-plans/106e2-pairing-transport-sas.md` §0a. **Shipped:** NEW `pangolin-core::pairing_transport` — fixed-layout zero-serde codec for the QR / copy-paste-string pairing payload (`DOMAIN ‖ schema_version(2) ‖ vault_id(32) ‖ device_id(32) ‖ x25519_pairing_pub(32) ‖ signer(20) ‖ freshness_nonce(16)`; `PAYLOAD_LEN` byte-pinned; base32 + 4-byte truncated-SHA-256 checksum text form for camera-less devices); `pangolin-crypto::pairing` SAS additions (`SAS_DOMAIN = "pangolin-pairing-sas-v0"`, `Sas`, `SAS_FRESHNESS_NONCE_LEN`, `derive_sas` = SHA-256 over lex-sorted pubkeys + nonce, mod 1e6, 6 zero-padded digits — canonical-symmetric L3); 5 new pub `pangolin-store::Vault` methods (`seal_vdk_for_new_device`, `device_pairing_pubkey`, `device_pairing_device_id`, `install_paired_vdk` — ATOMIC single-`unchecked_transaction()` meta-write + device-key reseal, so B's vault adopting A's `vault_id` is all-or-nothing; `open_paired_vdk_seal`); new `pangolin-chain::EvmWallet::from_signer` constructor; 6 NEW `#[uniffi::export]` bindings in `pangolin-ffi::pairing` — `pairing_begin_new_device` (B's payload + fresh nonce), `pairing_local_payload(their_nonce)` (A's mirror bound to B's nonce, so SAS matches), `pairing_decode_bytes`/`_string`, `pairing_derive_sas` (rejects mismatched nonces), `vault_add_device` (cross-vault gate ⇒ Validation BEFORE the RPC; reads live `deviceNonce` ⇒ EIP-712 sign ⇒ on-chain `addDevice` ⇒ seal VDK store-side ⇒ record directory entry GAP-A ⇒ return non-secret envelope), `pairing_open_and_join` (open seal ⇒ install new VDK + adopt vault_id atomically). **Surprises / parent-side fixes:** the builder's `tests/anvil_pairing_e2e.rs` (~260 lines) needed crate-private `lock_vault` access which integration-test crates can't see by design — rather than relax `lock_vault` to `pub` (broader API surface) or rewrite the imports, deleted the file and flagged it as a follow-up. The builder also stranded a few lint cleanups for the parent: 1 unused import, 1 `_secret` underscore-prefixed binding renamed to `secret` (it WAS used via `drop(secret)`), 1 redundant `.clone()`, 2 test-only `significant_drop_tightening` allows on a seal/open round-trip test, 1 `usize→u8` truncation in a test (replaced with `u8::try_from(i).expect(...)`), 1 `(buf & mask) as u8` simplified to a direct `!= 0` check, plus module-level `#![allow(clippy::doc_markdown, too_long_first_doc_paragraph, doc_lazy_continuation)]` on the two heavily-documented modules (`pairing_transport.rs` + `pairing.rs` FFI). **Audit CLEAN** on the central checks: L1 zero-secret-crosses-FFI confirmed per-binding (the X25519 pairing SECRET stays inside the store; the VDK travels engine-side between `open_paired_vdk_seal` and `install_paired_vdk` by VALUE — never as FFI bytes; passwords cross only as opaque `SecretPassword`; outputs are pubkeys/ids/signer/nonce/sealed-envelope-bytes/SAS-string, all non-secret; Debug impls verified to redact); L2 the SAS defeats a pubkey-swap MITM (the load-bearing test `derive_sas_defeats_pubkey_swap_mitm_through_ffi` exercises through the FFI surface — swapped pubkey ⇒ SAS differs); L3 canonical-symmetric (lex-sorted pubkeys ⇒ identical code on both devices regardless of role); L4 codec is fixed-layout / zero-serde / version-gated / checksummed and the panic-hunt found ZERO panic paths on host input; L5 freshness/replay (CSPRNG-generated nonce + `pairing_derive_sas` rejects mismatched nonces + on-chain `deviceNonce` defends chain-replay); `install_paired_vdk` atomicity verified (single tx wraps both writes); cross-vault rejection happens BEFORE the on-chain call. **2 LOW non-blocking follow-ups:** (i) no fault-injection test for `install_paired_vdk` (code IS atomic; future refactor that splits the tx would silently pass tests — recommended unit test mocks a write failure); (ii) restore the deleted anvil FFI E2E for the composed full-stack add-device path (every component IS anvil-tested elsewhere — chain via #106c, gates via unit tests, crypto via the seal/open round-trip — but the composed-through-FFI path is gap). **Observation (NOT a finding):** the SAS binds the two pairing pubkeys + nonce but NOT signer/device_id; a MITM substituting just the signer field would pass SAS but seal-binding still prevents VDK theft (impact bounded to DoS/gas-burn, not confidentiality). §0a Q-d explicitly chose this tradeoff. **Gates (host — the build/audit agents are cargo-sandbox-blocked):** `cargo fmt --check` clean; `cargo clippy -p pangolin-{core,crypto,store,ffi} --all-targets -- -D warnings` CLEAN under both default and `--features integration-tests`; `cargo test --workspace` green (incl. the workspace `no_empty_ignored_tests` meta-test + the new pairing_transport 13 tests / SAS 7 tests / FFI-pairing 15 tests — including the L2 anti-MITM, L3 canonical-symmetric, cross-vault rejection, locked-before-chain, the seal/open round-trip end-to-end). `cargo tree -p pangolin-{core,store,crypto} | grep -ci uniffi` = 0 (the cardinal isolation invariant holds — uniffi stays exclusively in pangolin-ffi). NO new external crates (sha2 already in workspace tree), uniffi pinned `=0.31.1`, `forbid(unsafe)`, AGPL SPDX on new files. **#106 multi-device epic now COMPLETE end-to-end** (#106a contract → #106b-1 pairing crypto → #106b-2 VDK rotation → #106c device-add client → #106c2 V2 data-plane → #106d revocation gate → #106e-0 composition layer → #106e-0b guardian onboarding → #106e-1 recovery/rotation FFI → #106e-2 pairing transport + device-add FFI). Stays TESTNET-ONLY until D-011. **Remaining project work:** #107 (V1 read-topic bug, standalone) + the 2 LOW follow-ups above.

## 2026-05-22 · MVP-3 issue #106e-1 — recovery + rotation FFI entry points (the thin uniffi layer over #106e-0 + #106e-0b)

Plan `docs/issue-plans/106e1-ffi-entry-points.md` §0a (Kelvin sign-off 2026-05-22). SUBSUMES the long-pending #105b recovery-FFI. **Shipped (5 `#[uniffi::export]` bindings in pangolin-ffi):** `vault_pending_rotations` (reads the crash-durable rows); `vault_complete_rotation(handle, master_password, config)` — per §0a Q-b the ENGINE reads the live `RevisionLogV2` authorized set itself via `read_authorized_set_v2` (async `block_on_local`, FAIL-CLOSED — any chain error ⇒ `FfiError::Chain`, never proceeds with an empty/partial set); `vault_guardian_open_share` — returns the opened `Share` as an opaque `Arc<FfiOpenedShare>` Object (the `SecretPassword` template: zeroizing, exposes ONLY `byte_length()`, `into_inner` crate-private, Debug-redacted); `vault_recover_from_shares` (LOST-EVERYTHING — all material host-supplied as length-validated `Vec<u8>`/`u64`, works on a loaded-but-Locked vault); `vault_onboard_guardians` (the #106e-0b binding folded in here per Q-a). Plus `FfiRotationPending`/`FfiRotationResult`/`FfiRecoveryResult`/`FfiOnboardingResult`/`FfiGuardianRoster` records + `composition_error_into_ffi` (exhaustive `CompositionError → FfiError` mapping with authentication-class collapse to `Validation{kind:"authentication"}`). **Surprise (main-session fix):** the builder's `vault_complete_rotation` hit the RPC on a LOCKED vault and failed with `Chain` instead of failing fast with `Session` — added an active-session pre-check (`vault.state() != VaultState::Active`) BEFORE `block_on_local` so a locked vault returns `Session` with zero RPC. Builder otherwise finished cleanly (no stall this time). **Audit CLEAN (no findings)** on all 6 checks: L1 zero-secret-crosses-FFI confirmed per-binding (the only secret that leaves the engine is the opaque `Arc<FfiOpenedShare>` — there is no `Vec<u8>` getter on its `#[uniffi::export]` surface; passwords cross IN only as `Arc<SecretPassword>`; all outputs are epochs / merkle-context / unknown-survivor lists); `complete_rotation` is genuinely fail-closed (no `unwrap_or_default` / `.ok()` / empty-Vec fallback on the read path; gate-before-RPC); thin wrappers, no new atomic/crypto surface; every host-supplied byte input length-validated to `Validation{kind:"argument"}` — every production `.unwrap()` is total or test-gated, no panic reachable on a malformed blob; session-gating correct per binding (recover is the lost-everything exception); errors carry no secret + the mapping is exhaustive. One LOW non-blocking note (test name slightly over-promises vs what it mechanically catches — the real L1 guarantee rests on source review of the `#[uniffi::export]` block, which is correct). **Gates (host — agents cargo-sandbox-blocked):** `cargo fmt --check` clean; `cargo clippy -p pangolin-ffi --all-targets -- -D warnings` clean (fixed 5 lints: inclusive-range, doc-list-item, needless-range-loop, two `significant_drop_tightening` allows on test helpers — the codebase's existing pattern); `cargo test --workspace` green (78 ok-summaries, 0 failures) incl. the `no_empty_ignored_tests` meta-test + the new FFI binding tests (fail-closed-on-bad-rpc, locked-before-chain, opened-share-length-only, exhaustive-error-mapping, length-validation negatives, onboard→open→recover round-trip through the FFI). `cargo tree -p pangolin-core | grep -ci uniffi` = 0 (uniffi stays exclusively in pangolin-ffi — the L1 cardinal isolation). No new crypto; uniffi stays pinned `0.31.1`; the only Cargo addition is a `pangolin-store/test-utilities` dev-dep (test-only, no runtime impact). **Deferred:** #106e-2 (pairing transport + SAS, plan LOCKED, builds on top of this); the guardian-roster BACKUP FORMAT (recurring 6.x gap — host-supplied here); the standalone #107 V1-read-topic bug. Stays TESTNET-ONLY until D-011.

## 2026-05-22 · MVP-3 issue #106e-0b — production guardian-escrow ONBOARDING (the missing recovery prerequisite)

Plan `docs/issue-plans/106e0b-guardian-onboarding.md` §0a (Kelvin sign-off 2026-05-22). Split out of #106e-1: `complete_rotation`/`recover_from_shares` both need an onboarded escrow, but the only initial-escrow writer was the test-only `__test_onboard_recovery_escrow` — so the recovery surface was a dead end until production onboarding existed. **Shipped (Q-a = Option B, ONE shared primitive):** lifted the split-and-seal composition into `pangolin_crypto::escrow::onboard_escrow` (+ `EscrowOnboarding`/`EscrowAssignment`, upstream of both store + core); refactored `pangolin_core::recovery::onboard_guardian_escrow` (the rotation/recovery re-split) to a byte-PRESERVING wrapper over it (one implementation, no drift); added the production `pangolin_store::Vault::onboard_guardians(threshold, &[x25519_pub]) -> OnboardingOutcome{epoch}` (reads the active VDK store-internal, single-tx escrow write, genesis epoch, re-onboard replaces in place) + a test-only `__test_recovery_backup_material` reader for the round-trip. **Surprise:** onboarding has NO dep-arrow problem (unlike #106e-0) — it needs only `pangolin-crypto` (`split_rwk`/`seal_share`/`wrap_vdk_under_rwk`), so it's a pure store method. **Build note:** the builder agent stalled mid-verification with the code written; the main session finished it on the host (fixed a real L5-test bug — the test recovered into a *blank* vault then asserted it could read the owner's pre-onboard account, impossible; rewritten to recover the owner vault IN PLACE after locking, the faithful lost-everything-with-backup scenario + the only meaningful byte-identity proof). NEW hermetic `onboard_recover_roundtrip.rs`: production onboard → guardians open shares → recover in place → the recovered VDK decrypts a pre-onboard account (byte-identical) + the vault is usable. **Audit CLEAN (no findings)** — byte-preservation confirmed field-by-field (epoch encoding `[0;8]‖be(u64)`, share ordering, `WrapContext`, RWK/wrap/split); secret hygiene (RWK+shares dropped in `onboard_escrow`, active VDK borrowed store-internal, only the non-secret epoch out), single-tx atomicity, replace-on-re-onboard, bounds all pass. **Gates (host — the build/audit agents are cargo-sandbox-blocked):** `cargo fmt --check` clean; `clippy -p pangolin-crypto -p pangolin-core -p pangolin-store --all-targets -- -D warnings` clean; `cargo test --workspace` green incl. the extraction-sensitive `recovery::orchestration`/`rotation` proptests + the `no_empty_ignored_tests` meta-test; recovery-escrow coupled + device-rotate + complete-rotation anvil E2Es green against live RevisionLogV2/RecoveryV1. **Process miss recorded:** #106e-0's merge initially reddened main on the `rustfmt` CI job because the pre-merge host gate ran tests/clippy/anvil but not `cargo fmt --check` (the hand-written hermetic tests weren't fmt-canonical); fixed by `d261843`, gate now includes fmt. NO new crypto, NO new deps. **Deferred:** the guardian-roster BACKUP FORMAT (recurring 6.x gap — material is host-supplied here); the `vault_onboard_guardians` FFI binding (lands with #106e-1); the on-chain guardian SET (`set_guardian_set_v1`, #103, host-driven). Stays TESTNET-ONLY until D-011.
## 2026-05-22 · MVP-3 issue #106e-0 — store/core COMPOSITION LAYER (the public entry points the #106e-1 FFI needs)

Plan `docs/issue-plans/106e0-composition-layer.md` §0a + the **0a-CORRECTION** (Kelvin sign-off 2026-05-22). The missing-middle slice between the audited drivers/commits and the (unbuilt) thin #106e-1 FFI: public composition entry points that compose the already-audited primitives so the FFI never juggles a secret. **Surprise (builder caught it, plan-gate correction):** the LOCKED Q-a put all three methods on `Vault` in `pangolin-store`, but the dep arrow is one-way `pangolin-core → pangolin-store` — a store `Vault` method physically cannot call the `pangolin-core` rotation/recovery drivers (Cargo cycle). RESOLUTION (verified secret-clean): `complete_rotation` + `recover_from_shares` are `pangolin-core` free fns over `&mut Vault` (core can reach both the core drivers AND store's pub commits); `guardian_open_sealed_share` stays a store `Vault` method (crypto-only, upstream). **Shipped:** NEW `crates/pangolin-core/src/composition.rs` — `complete_rotation(&mut Vault, master_password, current_onchain_set) -> RotationOutcome { new_epoch, unknown_survivors }` (reads the local survivor directory + non-secret escrow params store-side → `rotate_vdk_for_survivors` mints the fresh VDK → the audited `commit_vdk_rotation_from_active` → resolves every retired rotation-pending row, Q-e) and `recover_from_shares(&mut Vault, wrapped_recovery, opened_shares, roster, new_password, current_epoch, vault_id) -> RecoveryOutcome` (LOST-EVERYTHING only; all material host-supplied, pulls nothing from `self.active`; `recover_vdk_from_shares` → the audited `commit_recovery_rekey`); plus `RotationOutcome`/`RecoveryOutcome`/`GuardianRoster`/`CompositionError`. In `pangolin-store/src/vault.rs`: `Vault::guardian_open_sealed_share` (derives the guardian X25519 SEALING secret from the active `DeviceKey`, `open_sealed_share`; the opened `Share` is the ONLY secret out — FFI wraps it opaque), the thin pub `Vault::commit_vdk_rotation_from_active` (promoted from the `__test_` helper; `self.active.take()` → the audited `commit_vdk_rotation`, keeping `old_vdk`/`device_key` store-internal — no new atomic surface), and the non-secret `Vault::recovery_escrow_params` accessor (opens the escrow store-side, returns only `(t,M)`+guardian pubs+epoch; the active VDK never crosses the crate boundary) + `__test_onboard_recovery_escrow`. **Audit CLEAN** on all 6 security checks (secret hygiene / `old_vdk` never crosses into core / `new_vdk`+recovered VDK consumed-and-dropped / lost-everything scope / Q-e incl. the dangerous "don't clear a still-in-set signer" direction / guardian-open uses the sealing derivation + fails closed on wrong vault_id/epoch). One **LOW** finding — `complete_rotation`'s GAP-A + Q-e logic was only exercised by the `#[ignore]`'d anvil E2E — closed by a fix-pass: NEW hermetic `composition_rotation.rs` (GAP-A unknown-survivor surfacing + Q-e retired-pending clearing + a discrimination companion that a still-in-set signer's pending row survives), both proven to bite (breaking the Q-e guard / dropping the unknown list each turns them RED). NEW hermetic `composition_recovery.rs` (lost-everything round-trip, below-threshold rejection commits nothing, wrong-epoch guardian-open fails closed). Anvil E2E extended with `complete_rotation_public_composition_e2e_against_anvil`. **Gates (run on host — the build/audit agents were cargo-sandbox-blocked):** `cargo test --workspace` exit 0, 0 failures, the workspace `no_empty_ignored_tests` meta-test green (only 2 pre-existing cli stdin tests skipped — non-interactive-stdin environmental, pass in CI); `clippy -D warnings` clean on both crates (forced full re-lint); `bash scripts/anvil-ci.sh setup` + the 3 device E2Es (`complete_rotation_public_composition` / `device_add_remove_rotate` / `revocation_honor_gate_remove_then_read`) PASS against the live-deployed RevisionLogV2 bytecode. NO new crypto, NO new deps, NO new atomic surface. **Deferred:** #106e-1 (thin FFI, subsumes #105b) → #106e-2 (pairing transport + SAS); the guardian-roster BACKUP FORMAT for lost-everything recovery (recurring 6.x gap — host-supplied param here); the standalone #107 V1-read-topic bug. Stays TESTNET-ONLY until D-011.

## 2026-05-20 · MVP-3 issue #103 — Recovery v1 CLIENT (chain-client control plane: merkle + EIP-712 approvals + lifecycle broadcast + anvil time-warp test)

Plan at `docs/issue-plans/103-recovery-client.md` + `DECISIONS.md` #103 (R-where/R-merkle/R-test/R-guardians/R-anvil, Kelvin sign-off 2026-05-20). The Rust chain-client half of the highest-risk EPIC; drives the merged `RecoveryV1` contract (#102). Scope = chain-client control plane ONLY (NO VDK escrow, NO revocation-on-read — those are Workstream B / #103-C, deferred). **Shipped (3 new modules, zero new deps):** (1) NEW `crates/pangolin-chain/src/recovery_signing.rs` (AGPL SPDX) — the `Approve` EIP-712 machinery: `build_domain_recovery` (domain name `"Pangolin Recovery"`, version `"1"`), `approve_struct_hash`, `approve_digest`, `build_signed_approval_v1` (off-chain guardian signer), `recover_approver_v1`; pinned `APPROVE_TYPEHASH_V1` (`2d50d9bb…24113`, re-keccak'd by a hermetic test against the contract's literal at `RecoveryV1.sol:330-332`) + `RECOVERY_DOMAIN_SEPARATOR_ANVIL_DEV_V1` (anvil-derived, with a documented TODO to capture a `RECOVERY_DOMAIN_SEPARATOR_BASE_SEPOLIA_V1` + `EXPECTED_RECOVERY_ADDRESS_BASE_SEPOLIA` once a testnet deploy exists); **REUSES `eip712_digest` + `is_canonical_s` verbatim from `secp256k1_signing.rs`** (one digest impl in the crate — L3 no silent-drift surface). (2) NEW `crates/pangolin-chain/src/recovery_client.rs` (AGPL SPDX) — the `sol!` RecoveryV1 binding (5 mutators + 2 views + `hashApprove` + 5 events); the **hand-rolled merkle builder** (`guardian_leaf` = `keccak256(abi.encode(addr))` = 12-zero-pad ‖ 20-byte addr, NOT encodePacked; `build_guardian_root` + `build_membership_proof` + `verify_membership_proof`, sorted-pair-keccak, OZ-StandardMerkleTree-compatible, NO crate dep — L9); the 5 lifecycle broadcast fns (`set_guardian_set_v1`/`initiate_recovery_v1`/`approve_recovery_v1`/`cancel_recovery_v1`/`finalize_recovery_v1`) over ONE shared `broadcast_call` retry loop mirroring `chain_submit.rs`'s R-c taxonomy + EIP-1559 envelope + 50-gwei cap + 1-conf receipt-await + a private `resolve_envelope_chain_id` (#101 L4 verbatim); a `RecoveryAnchorV1` receipt-decode (generic over the typed event); `read_live_attempt_v1` + `build_live_approve_fields_v1` reading the LIVE `attemptNonce`+`proposedAuthority` (L11 anti-replay — refuses to build a stale-attempt digest); `read_vault_authority_v1`. `approve_recovery_v1` carries the guardian's 65-byte off-chain sig + the merkle proof + a client-side proof pre-flight. (3) FILLED `crates/pangolin-core/src/recovery/mod.rs` (AGPL SPDX) — pure no-secret no-chain domain types: `RecoveryStatus` (u8-mirror of the contract's `Status` enum), `bounds` (MIN_DELAY_SECS=259_200, threshold/guardian bounds), `GuardianSetConfig::validate` (pre-flights the contract's `setGuardianSet` bounds). NO uniffi (Q3). **Extended `scripts/anvil-ci.sh`** (R-anvil): deploys RecoveryV1 (via the existing `DeployRecoveryV1.s.sol`) + adds a `dev.json` RecoveryV1 entry + runs the lifecycle test; the recovering wallet reuses the already-funded `[0x42;32]` seed (guardians sign off-chain, need no funding). **The anvil Rust↔contract lifecycle test (the CENTERPIECE, `#[ignore]`+`integration-tests`-gated):** deploy → `setGuardianSet`(real 3-of-5 merkle root) → `initiateRecovery` → `approveRecovery×3` (real guardian EIP-712 sigs + real merkle proofs, **accepted by the LIVE `_verifyMerkleProof`**) → `cast rpc evm_increaseTime 259200` + `evm_mine` → `finalizeRecovery` → asserts `RecoveryFinalized` + the rotated `vaultAuthority`. Negative gates: finalize-below-threshold, finalize-before-delay, non-authority-cancel, duplicate-approval — all revert. **PROVEN (L10):** `bash scripts/anvil-ci.sh all` → all in-scope tests PASS incl. `recovery_lifecycle_against_anvil` (real proofs accepted, real approvals recovered, 72h time-warp rotates authority). **L10 regression-proof (the load-bearing audit step):** (a) temporarily switched `guardian_leaf` to `encodePacked` (20-byte) instead of `abi.encode` (32-byte) → re-ran → `approveRecovery` went RED (`RevertedPreBroadcast` — the live `_verifyMerkleProof` rejected `ErrInvalidMerkleProof`) → REVERTED; (b) temporarily corrupted the last byte of `APPROVE_TYPEHASH_V1` → re-ran → `approveRecovery` went RED (live `_recover` recovered a wrong signer → `ErrInvalidSignature`) → REVERTED. A broken merkle leaf / digest does NOT silently pass. **Hermetic byte-pins (9 in chain, all green):** `approve_typehash_matches_pinned_constant`, `recovery_domain_separator_matches_pinned_anvil_value`, `sign_recover_round_trip`, `wrong_chain_id_recovers_different_signer`, `signature_shape_is_canonical`, `merkle_root_matches_hand_computed_fixture`, `every_guardian_proof_verifies`, `non_member_has_no_proof_and_fails_verify`, `guardian_leaf_encoding_is_abi_encode_not_packed`; +4 core recovery tests. **Gate all green:** `cargo fmt --all --check`; `cargo clippy -p pangolin-chain -p pangolin-core --all-targets -D warnings` (+ solo `-p pangolin-chain --all-targets` + `--features integration-tests`); `cargo test -p pangolin-chain --lib` (185) + `-p pangolin-core --lib` (20); `cargo test -p pangolin-chain` integration (`deployment_json_pins_match_rust_constants` still 5/5 green; 26-test live suite green); `bash -n scripts/anvil-ci.sh`; `bash scripts/check-no-ring.sh` + `check-chain-no-store.sh` (0 store edges — L7); pangolin-crypto tree 0 `serde `, pangolin-core tree 0 `uniffi`; `cargo deny check advisories` ok; `forge build --root contracts --deny-warnings` clean (RecoveryV1 + DeployRecoveryV1 build). **L5 confirmed:** no VDK / guardian secret key touched — guardians sign `Approve` off-device, #103 carries only the 65-byte sig + merkle proof. **Zero new external deps (L9):** alloy provides keccak/sol!/EIP-712/secp256k1; merkle hand-rolled. **Deferred:** Workstream B / #103-B (guardian-escrow / threshold-VDK-recovery crypto — the Option-2 heart); #103-C (revocation-on-read); the BaseSepolia `RECOVERY_DOMAIN_SEPARATOR_*` + `EXPECTED_RECOVERY_ADDRESS_*` testnet capture (+ wiring into `deployment_json_pins_match_rust_constants`) once RecoveryV1 lands on Sepolia; live-testnet `#[ignore]` lifecycle tests; 6.x UX; THREAT_MODEL recovery-client entry (post-audit).

---

## 2026-05-20 · MVP-3 issue #102 — RecoveryV1 social-recovery contract (merkle guardians, 72h delay, authority rotation)

Plan at `docs/issue-plans/102-recovery-v1-contract.md` + `DECISIONS.md` #102 (R-a..R-h, Kelvin sign-off 2026-05-20). HIGHEST-RISK EPIC; external-audit-gated before mainnet (D-011). The project's FIRST stateful on-chain state machine. **Shipped:** NEW `contracts/src/RecoveryV1.sol` (AGPL SPDX; exact-pin solc 0.8.24, evm shanghai; runtime 5,370 bytes, well under EIP-170). Per-vault lifecycle `None → initiateRecovery → PENDING → approveRecovery×N → finalizeRecovery → FINALIZED | cancelRecovery → CANCELED`. Five external mutators (`setGuardianSet`, `initiateRecovery`, `approveRecovery`, `cancelRecovery`, `finalizeRecovery`) + three view digest-oracles (`hashApprove` consumed; `hashCancel`/`hashInitiate` forward-compat). Storage: `guardianSet[vaultId]` (`bytes32 root` + `uint8 threshold` + `uint8 guardianCount` + `bool initialized`), `vaultAuthority[vaultId]`, `recovery[vaultId]` (`proposedAuthority`/`initiatedAt`/`attemptNonce`/`approvals`/`Status`), `hasApproved[vaultId][attemptNonce][guardian]`, immutable `DOMAIN_SEPARATOR`. 12 custom errors; 5 events each carrying `uint16 schemaVersion` (L5; `MAX_KNOWN_SCHEMA_VERSION=1`). **R-a/R-b/R-c/R-d/R-e/R-f/R-g/R-h implemented exactly:** guardian set = merkle root with a standard sorted-pair-keccak `_verifyMerkleProof` (leaf = `keccak256(abi.encode(guardian))`; approval requires BOTH the merkle proof AND the recovered EIP-712 signer == the proven leaf); `MIN_DELAY = 72 hours` fixed constant (finalize reverts `ErrDelayNotElapsed` unless `block.timestamp >= initiatedAt + MIN_DELAY`); approval auth = EIP-712 `Approve(bytes32 vaultId,address proposedAuthority,uint64 attemptNonce,uint64 expiresAt,uint16 schemaVersion)` bound to the live attempt's `proposedAuthority`+`attemptNonce` (per-attempt dedup → `ErrDuplicateApproval`; `expiresAt` anti-stale); authority = `mapping vaultId→address`, rotated ONLY in finalize's success path; guardian set immutable (`ErrGuardianSetAlreadyInitialized` on 2nd `setGuardianSet`); one-active-per-vault (`ErrRecoveryAlreadyPending`); cancel = authority-only via `msg.sender == vaultAuthority` (cleaner than a cancel-sig); EIP-712 domain bakes `chainId`+`address(this)`. L8 bounds contract-enforced (`MIN_THRESHOLD=2`/`MAX_THRESHOLD=9`/`MIN_GUARDIANS=3`/`MAX_GUARDIANS=15`/`threshold≤count` → `ErrThresholdOutOfBounds`/`ErrGuardianCountOutOfBounds`). `_recover` copied VERBATIM from RevisionLogV1 (len-65, v∈{27,28}, canonical-s, reject signer==0). **Self-bootstrap choice (the one detail the spec left open):** `setGuardianSet` self-bootstraps `vaultAuthority[vaultId] = msg.sender` — the bootstrapping device is by definition in the user's control, so it is the right genesis trust anchor; mirrors RevisionLogV1's R-b first-publisher self-register; NOT a security-significant fork (any model needs SOME genesis authority and `msg.sender` is minimal-surface). `initiateRecovery`/`finalizeRecovery` are intentionally permissionless (security is the quorum+delay, not msg.sender gating; matches the lost-all-devices case). **L2 cardinal rules verbatim+stricter CONFIRMED:** no admin/owner/upgrade/proxy/pause/selfdestruct/delegatecall/receive/fallback; non-payable; ZERO external calls (`call`/`transfer`/`send`/`delegatecall`) → zero reentrancy surface; L12 — no `bytes` blob storage, no VDK/secret on chain, rotates a plain 20-byte address only. **Tests:** NEW `contracts/test/RecoveryV1.t.sol` (50 unit tests: full happy-path lifecycle with `vm.warp` past 72h, every revert path, exact-delay-boundary success, cross-chain replay via fake-domain sig, wrong-proposed-authority binding, stale-attempt-nonce replay, all 4 merkle leaves verify + forged-proof reject, signer/leaf mismatch, the no-admin selector probe over 20 forbidden selectors incl. `forceFinalize`/`adminCancel`/`setAuthority`/`setThreshold`/`pauseRecovery`/`removeGuardian`/`updateGuardianSet`, non-payable ETH-reject, `_recover` discipline incl. high-s/bad-v/zero-r-zero-s, digest-oracle parity, multi-vault isolation, authority-follows-rotation cancel) + NEW `contracts/test/RecoveryV1.invariant.t.sol` (10 invariants at the foundry.toml 10k×32 with `fail_on_revert`+`vm.assume` handler discipline mirroring RevisionLogV1.invariant.t.sol: `noFinalizeBeforeDelay`, `noFinalizeBelowThreshold`, `canceledIsTerminal`, `oneActiveRecoveryPerVault`, `guardianApprovalDedup`, `thresholdAlwaysInBounds`, `authorityOnlyRotatesViaFinalize` via a ghost-authority cross-check, `noStorageMutationBesidesWhitelist`, `onlyKnownEventsEmitted`, `noVDKLikeDataOnChain`). The handler IS the vault authority + drives initiate/approve/approveOutsider/cancel/warpAndFinalize/warp; the catch branch asserts no event on revert (L6) + authority-unchanged on failed finalize. **Also shipped:** `contracts/script/DeployRecoveryV1.s.sol` (no constructor args; mirrors DeployRevisionLogV1); `contracts/abi/RecoveryV1.json` (committed for the ABI-drift gate); `.github/workflows/ci.yml` ADDITIVELY extended with the RecoveryV1 deploy dry-run + slither + ABI-drift steps (existing v0/v1/entitlement steps UNTOUCHED). **Forge gate (local):** `forge build --deny-warnings --sizes` clean (runtime 5,370 < 24,576); `forge fmt --check` clean; `forge test` all pass (50 unit + 10 invariants @ 10k×32, 0 reverts); `slither src/RecoveryV1.sol --fail-high` exit 0 (4 informational findings: timestamp-comparison [intended, the delay], assembly [the verbatim `_recover`], DOMAIN_SEPARATOR naming [matches EntitlementRegistry] — zero high-severity); ABI-drift clean (node `JSON.stringify(abi,null,2)+'\n'` byte-identical to the CI Python formatter, verified against the committed EntitlementRegistry.json). **ZERO Cargo/Rust changes (L15):** nothing under `crates/`/`apps/`/`Cargo.*`. **No genuine fork hit beyond the self-bootstrap detail** (resolved per the prompt's "sensible self-bootstrap is within scope"). **Deferred (per plan):** testnet deploy + mainnet (external-audit-gated, D-011); recovery CLIENT logic (Rust, parallel) + VDK re-wrap (6.5) + old-device-key revocation-on-read (R-d gap, 6.5); anvil Rust↔contract lifecycle tests (with the client cycle — Cargo impact zero here per L15); ZK guardian privacy (R-a v2+); mutable guardian sets (R-e); per-vault configurable delay (R-b v2); THREAT_MODEL RecoveryV1 entry (post-audit).

---

## 2026-05-20 · Issue #101 — anvil-fork CI harness + scoped Dev signing chain-id (MVP-3 foundation)

Plan at `docs/issue-plans/101-anvil-fork-ci.md` + `DECISIONS.md` #101 (R-a..R-e) + the #101 amendment (Kelvin sign-off 2026-05-20). Closes the env-quirk #14 gap before the highest-risk MVP-3 Recovery contract: hermetic mocks fabricate receipts without running the contract's hash/sig logic, so the 3.3 `keccak(encPayload)`-vs-preimage calldata bug passed the full suite and was caught only by adversarial audit. **Shipped (harness):** NEW `scripts/anvil-ci.sh` (AGPL SPDX; subcommands setup/run/teardown/all) — starts `anvil --silent` (chain-id 31337, port 8545), POLLS `cast block-number` for readiness (bounded 60×0.25s, fail-closed, NO fixed sleep — L5), `trap`-teardown on any exit, deploys RevisionLogV1 + EntitlementRegistry via the existing forge scripts with anvil's acct[0] public test key, parses addresses + deploy-blocks from the **structured broadcast artefact** `contracts/broadcast/<Script>.s.sol/31337/run-latest.json` via `jq` (env-quirk finding: `forge script`'s human log has NO `Contract Address:` line on the anvil path — parse the artefact's `.transactions[].contractAddress` + `.receipts[].blockNumber`), generates `contracts/deployments/dev.json` in the shape `load_deployed_address` walks (`.contracts.<Name>.address`), resolves `fixed_wallet()`'s EVM address via a new `print_fixed_wallet_address` helper test + funds it 1 ETH via `cast rpc anvil_setBalance`, then runs the 3 in-scope tests in dev mode. NEW additive `anvil-integration` job in `.github/workflows/ci.yml` (ubuntu-latest, submodules recursive, `foundry-toolchain@v1` at the SAME pinned `v1.0.0` as the contracts jobs — L4; calls `bash scripts/anvil-ci.sh all`, not inline shell — env-quirk #16; existing jobs UNTOUCHED — L3). NEW `crates/pangolin-chain/src/test_env.rs` test seam (`target_chain_env`/`rpc_url`/`resolve_signing_chain_id`/`require_or_fail`, reading `PANGOLIN_CHAIN_ENV=dev|base-sepolia`, default base-sepolia) gated to test/test-utilities/integration-tests builds; wired into the 3 in-scope `#[ignore]` tests (`publish_v1_live_d017_smoke` chain_submit.rs, `live_pull_once_against_d017_advances_checkpoint` pangolin-store/tests/pull_live.rs, `live_balance_query_against_d017_wallet` chain integration.rs; integration.rs's `deployment_path()`/`rpc_url()` un-hardcoded). **L6 (load-bearing):** in dev mode the skip-clean `return` becomes a HARD error (`require_or_fail` panics, balance test `assert!`s) — a missing dev.json/RPC/env var turns CI RED, not skip (the 3.3 bug "passed" because the live test skipped). `.gitignore`: `contracts/deployments/dev.json` (R-e: runtime-generated, never committed; `contracts/broadcast/` was already ignored). **Shipped (scoped signing change — #101 amendment):** the publish test couldn't pass against anvil because the signing path hardcoded chain-id 84_532 (envelope) + `unwrap_or(0)` (EIP-712 domain) while anvil is 31337 and the contract bakes `block.chainid` into its domain separator. Threaded an explicit `chain_id: u64` into `secp256k1_signing::build_domain` (replacing `env.chain_id().unwrap_or(0)`), `build_signed_revision_v1`, `recover_signer_v1`/`recover_signer_v1_raw`, `Vault::sign_revision_v1`, and the 3 EIP-1559 broadcast paths in `chain_submit.rs` (new `resolve_envelope_chain_id` replacing the hardcoded `signed_revision_chain_id()`). **Caller resolves the id:** `env.chain_id()` when `Some` (BaseSepolia → 84_532, UNCHANGED, no RPC read) → use it; when `None` (Dev) → read `eth_chainId` from the provider. `Dev.chain_id()` STAYS `None` (enum + `chain_env_chain_ids_are_pinned` test untouched — L2). **Security:** production (BaseSepolia) NEVER reads its signing chain id from an untrusted RPC; only the Dev/local-anvil path does (doc-comment makes this explicit; `resolve_envelope_chain_id` reads the RPC for a fixed env ONLY to cross-check, then returns the PINNED id). **PROVEN locally (L9):** `bash scripts/anvil-ci.sh all` → anvil ready (1 poll) → both contracts deployed → dev.json generated → wallet funded → all 3 tests PASS incl. `publish_v1_live_d017_smoke` (real contract `_recover` accepts the 31337-domain signature) + pull advanced checkpoint to block 3 + balance read ~1 ETH. **L9 regression-proof:** temporarily fed the 32-byte hash instead of the preimage (the exact 3.3 bug shape) into the publish calldata, re-ran the harness — `publish_v1_live_d017_smoke` went RED with `ReceiptMismatch { expected_signer: 0x7b64…, observed_signer: 0x913b… }` (contract recovered a DIFFERENT signer), then REVERTED. The harness would have caught the 3.3 bug. **BaseSepolia byte-identical:** all hermetic signing pins (`domain_separator_matches_pinned_constant`, `typehash_matches_pinned_constant`, the 3.6 baseline-signature pin in privacy/tests.rs) stay green — they pass 84_532 explicitly, byte-identical to the pre-#101 `unwrap_or(0)` form. **No new external crate dep (L7)** (anvil/forge/cast are binaries; jq is CI-provided); `forbid(unsafe_code)` survives; AGPL SPDX on the new script + test_env.rs. Docs: NEW `docs/architecture/anvil-ci-harness.md`; THREAT_MODEL.md note (env-quirk #14 class now CI-gated). **Deferred:** Recovery v1's own anvil tests (land with the contract); funder-service-in-CI; indexer/conflict/sync tests against anvil (R-a Option B); time-warp testing; multi-OS anvil (Linux-only first cut). **Surprise:** the recover path (`recover_signer_v1_raw` via chain_sync's `verify_signer_or_reject`) is sync + providerless, so it resolves `env.chain_id().unwrap_or(0)` internally (byte-identical to pre-#101) rather than threading the live id through the whole sync stack — the fresh-anvil pull test never reaches recovery (its test vault has no on-chain events), keeping the change scoped to the signing/broadcast path per the locked amendment.

---

## 2026-05-16 · MVP-2 issue 5.3 — Conflict detection + resolution plumbing (FFI list_conflicts + PullReport delta + ConflictReport enrichment)

Plan at `docs/issue-plans/5.3.md` Kelvin-approved (R-a..R-g baked + L1..L11 + L-section). Third §5.x issue; built directly on top of 5.2's `PullReport` posture and the existing 1.6 + P8 + P9 conflict machinery — ZERO change to `ingest_chain_revision` / `refuse_if_frozen` / canonical-head election / merge-revision build paths. Shipped: `crates/pangolin-store/src/conflict.rs` — enriched `ConflictReport` (breaking change: `heads: Vec<RevisionId>` → `branches: Vec<ConflictBranchSummary>` with per-leaf `device_id` / `observed_at_block` / `schema_version` / `is_tombstone` / `on_canonical_chain` / `parent`) + NEW `ConflictBranchSummary` / `ConflictSnapshot` / `ConflictDelta` records + 9 hermetic tests; `crates/pangolin-store/src/vault.rs` — rewritten `list_conflicts` body that builds the per-branch summary via `revision_graph` + new internal `read_observed_at_block` helper (prefers chain-sync's `observed_at_block`, falls back to `chain_block_number` from `mark_published` for self-publish round-trips) + NEW `Vault::snapshot_conflicts` (cheap two-SQL-read pair returning a `ConflictSnapshot`) + NEW `Vault::list_conflicts_since(&ConflictSnapshot)` accessor + NEW free `diff_conflict_snapshots` pure helper + extended `pull_once` with pre-tick + post-tick snapshot computation feeding the new `PullReport` fields; `crates/pangolin-store/src/pull.rs` — extended `PullReport` with three new fields (`newly_frozen_accounts` / `newly_forked_accounts` / `newly_resolved_accounts` computed as directional set-differences) + 8 new hermetic tests including the MANDATORY R-b regression `pull_after_local_publish_does_not_self_freeze`; `crates/pangolin-store/src/lib.rs` — re-exports `ConflictBranchSummary` / `ConflictDelta` / `ConflictSnapshot`; `crates/pangolin-store/tests/conflict_live.rs` — NEW 105-LoC `#[ignore]`'d shape-only two-vault simulation test (env-quirk #14 defense; fixture-capture is the follow-up); `crates/pangolin-core/src/lib.rs` — re-exports the new conflict types so the FFI surface can name them; `crates/pangolin-ffi/src/revision.rs` — NEW `vault_list_conflicts` UniFFI entry point + NEW `FfiConflictReport` / `FfiConflictBranchSummary` records (32-byte `RevisionId` blobs flattened to `Vec<u8>` per existing FFI convention; L2 metadata-only + no AEAD-open path); `crates/pangolin-ffi/src/lib.rs` — re-exports. NEW `docs/architecture/conflict-surface.md` (~200 LoC) covering what 5.3 ships + per-cycle semantics + canonical host scheduler reaction loop + relationship to P9 + relationship to 5.4 + L1..L11 invariant preservation table + threat-model cross-ref; extended `docs/architecture/chain-sync.md` with the §Conflict-surface cross-ref; new "Conflict surfacing (5.3)" deep-dive section in `THREAT_MODEL.md` enumerating 8 per-surface threats (L-self-fork-on-publish, L-byte-flip-on-frozen-row-via-FFI, L-conflict-surface-leaks-frozen-payload, L-PullReport-delta-overcounts-on-existing-frozen, L-self-resolve-loopback, L-conflict-surface-races-with-resolve, L-list-conflicts-perf-on-N-1000-frozen, L-FFI-binding-bypasses-freeze-guards); appended R-a..R-g entries to `DECISIONS.md`. **No schema change, no `format_version` bump, no new external crate dep (L7), `forbid(unsafe_code)` survives.** **Resolved decisions R-a..R-g** (Kelvin sign-off 2026-05-16 — all four surfaced plan-gate recommendations taken; Q-b self-publish defense + Q-f auto-resolve + Q-g test surface defaulted to plan-gate defaults): R-a confirm existing P8 CRIT-1 freeze trigger (ZERO ingest_chain_revision changes); R-b confirm 5.1 inline-anchor-stamp + idempotency arm #1 (mandatory regression test PASSED first run); R-c bake per-tick delta into `PullReport`; R-d enrich `ConflictReport` with per-branch summaries (breaking change); R-e ship `vault_list_conflicts` FFI in 5.3; R-f defer auto-resolve heuristics; R-g hermetic + 1 live `#[ignore]`. **The MANDATORY regression test `pull_after_local_publish_does_not_self_freeze` PASSED.** Drives 5.1 `flush_publish_queue` against a `MockChainAdapter`, then replays the round-trip event through `ingest_chain_revision`, asserts `outcome == AlreadyPresent` AND `account_status().is_frozen_pending_resolve == false`. The Q-b Option B in-memory just-published set was NOT required. **Tests:** `pangolin-store --lib` 323 → 334 (+11; conflict.rs added 3 enrichment tests + 3 since-accessor tests = 6 new; pull.rs added 6 new tests including the regression; plus 1 PullReport-publicly-constructible test; the existing 6 conflict.rs tests had `heads.len()` calls renamed to `branches.len()` and one test was renamed `list_conflicts_handles_frozen_with_single_head` → `..._single_branch` for the new field name). `pangolin-cli --test two_vault_roundtrip` UNCHANGED (no CLI behaviour change in 5.3). Live `#[ignore]`'d test deferred to fixture-capture follow-up. **Plan-gate finding stood:** Grep confirmed `pangolin-cli` does NOT call `ConflictReport.heads` (resolve subcommand calls `Vault::account_heads` directly), so the R-d breaking change had zero downstream call-site updates outside the in-crate tests. **Surprises during build:** (1) Two test failures on first run — both in tests that used `resolve_fork` to "resolve" a freeze set up by `ingest_chain_revision`. The foreign event's payload is NOT AEAD-decryptable with the local VDK (different device key) so `resolve_fork` returned `AuthenticationFailed`. Fix: switch to `clear_frozen` (the metadata-only freeze-flag clear) for those tests; `resolve_fork`'s decrypt requirement makes it the wrong primitive for foreign-event scenarios. (2) `observed_at_block` source-of-truth ambiguity — the column is populated only by `ingest_pending_chain_revision` (chain-sync path); `mark_published` stamps `chain_block_number` instead. The enriched-summary fallback (prefer `observed_at_block`, fall back to `chain_block_number`) keeps the host UI displaying *some* "first-seen-on-chain" anchor for self-publish round-trips. (3) The `RevisionGraph::ancestors` API takes the head being walked, not the leaf being queried; the `on_canonical_chain` computation reuses `RevisionGraph::is_on_canonical_chain` (which already implements the correct walk). **Open follow-ups (NOT blocking):** (a) CLI subcommands `pangolin-cli conflicts ls` / `pangolin-cli conflicts show <account-id>` — CLI-V1 batch; (b) 5.4 indicator state machine — sequential §5.x cycle; (c) live `#[ignore]`'d fixture capture for `live_two_device_concurrent_edit_creates_conflict_resolvable_via_resolve_fork` — same operational posture as 4.1/4.2/4.3/5.1/5.2; (d) auto-resolve heuristics — deferred to MVP-2 5.5+ at earliest after user testing per R-f.

---

## 2026-05-16 · MVP-2 issue 5.2 — Pull loop primitive (host-owned timer + per-cycle picker dispatch + NoActiveSession cancellation)

Plan at `docs/issue-plans/5.2.md` Kelvin-approved (R-a..R-f baked + L1..L10 + L-section). Second §5.x issue; built directly on top of 5.1's `flush_publish_queue` posture verbatim. Shipped: NEW `crates/pangolin-store/src/pull.rs` (566 LoC including 14 hermetic tests; hosts `PullReport { mode: SyncMode, sync_report: Option<SyncReport>, pulled_at_unix_ms: i64 }` + `PullError { NoActiveSession, Chain(ChainError), Store(StoreError) }` + the `PULL_INTERVAL_SECS_DEFAULT=60` / `_MIN=5` / `_MAX=3600` / `_ENV_VAR="PANGOLIN_PULL_INTERVAL_SECS"` constants); NEW `Vault::pull_once(rpc_url, env, &vault_id) -> Result<PullReport, PullError>` async primitive (re-picks via `select_sync_mode` per R-c, dispatches `Slow` to 4.1's `sync_from_chain` verbatim per L4, returns signal-only for `OfferFast` / `AlwaysFast` per L2 — engine NEVER spawns the indexer); NEW `Vault::resolve_pull_interval_secs()` + `resolve_pull_interval_secs_from(env_value)` helpers (R-b env-var clamp mirroring 5.1's `resolve_batch_window_secs` pattern verbatim); NEW `Vault::last_pull_at_unix_ms() -> Option<i64>` diagnostic accessor (5.4 prep); `ActiveState` extended with `last_pull_at_unix_ms: Option<i64>` (in-memory only; not persisted; reset on every unlock). NEW `crates/pangolin-store/tests/pull_live.rs` (133 LoC; `#[ignore]`'d live test against D-017 deferred to fixture-capture follow-up per 4.1/4.2/4.3/5.1 R-g posture). NEW `docs/architecture/pull-loop.md` (286 LoC; R-a host-owned timer rationale + canonical host scheduler loop body + SyncMode dispatch table + env-var override + R-d offline backoff + R-c diagnostic stamp + drain triggers matrix + UX contract for `OfferFast` + relationship to 5.1/5.4 + threat model cross-ref). **No schema change, no `format_version` bump, no new external crate dep, no FFI surface.** **Resolved decisions R-a..R-f** (Kelvin sign-off 2026-05-16 — all four surfaced plan-gate recommendations taken; R-d offline backoff + R-f test surface defaulted to plan-gate defaults): R-a host-owned timer (zero `tokio::spawn` surface preserved inside `pangolin-store` — verified by Grep); R-b const + env-var cadence clamped `5..=3600`; R-c re-pick per cycle (no cache-invalidation surface; flips take effect on next tick); R-d flat retry on chain error at next interval (host scheduler concern; 5.4 owns "Offline" indicator); R-e `PullError::NoActiveSession` cancellation discipline mirrors 5.1's `BatchFlushError::NoActiveSession` posture verbatim; R-f hermetic + 1 live `#[ignore]`. **Adapter-less API shape accepted (plan-gate weak recommendation):** `pull_once(rpc_url, env, &vault_id)` instead of threading a `ChainAdapter` through — because slow-mode delegates to `sync_from_chain` which takes raw `rpc_url`/`env`/`vault_id` and the OfferFast/AlwaysFast branches are signal-only. If 5.4's `SyncOrchestrator` later needs an adapter, the additive change is a second method. **Tests:** `pangolin-store --lib` 305 → 323 (+18; 14 in `pull::tests` + 4 baseline coverage tests for error display + From-impls; `pangolin-cli --test two_vault_roundtrip` UNCHANGED at 5/5; live `#[ignore]`'d test deferred). All gates green first-try after one fix-pass round for env-quirk #11 (rust 1.94's stricter `clippy::doc_markdown` + `clippy::doc_lazy_continuation` + `clippy::doc_overindented_list_items` + `clippy::future_not_send` — `pull_once` inherits `select_sync_mode`'s `!Send` future since `Vault` is `!Sync`; `#[allow(clippy::future_not_send)]` applied with the same rationale comment 4.4 used). **Gates all green:** clippy clean; fmt clean; HIGH-1 + Q3 + L7-chain + L7-indexer all 0 (no `pangolin-store` edges added to `pangolin-chain` or `pangolin-indexer`); `cargo deny check advisories` ok; `cargo audit` ok (2 allowed warnings, unchanged). **Surprises during pickup:** (1) `SyncMode` lives in `pangolin-store::vault`, not `pangolin-chain` — first build attempt routed through `pangolin_chain::SyncMode` which doesn't exist; fixed by importing from `crate::vault::SyncMode` in `pull.rs` and replacing `pangolin_chain::SyncMode::*` with `SyncMode::*` in `vault.rs` (one Edit with `replace_all`). (2) `sync_from_chain` returns `Result<SyncReport, StoreError>` where chain errors are wrapped in `StoreError::ChainSyncError(ChainError)` (per 4.1's `From<ChainError> for StoreError` impl); the `pull_once` body unwraps that wrapping in its `map_err` so `PullError::Chain(_)` cleanly distinguishes "RPC failure, host retries next tick" from `PullError::Store(_)` "true store error, host breaks." (3) `doc_lazy_continuation` required adding blank lines between numbered list items in the module doc (paragraph-per-item disambiguates the lazy-continuation rule from the overindent rule — both lints fire on different patterns of multi-line numbered items). **Open follow-ups (NOT blocking):** (a) FFI exposure of `pull_once` / `last_pull_at_unix_ms` — CLI-V1 batch; (b) `pangolin-cli pull` / `pangolin-cli pull-status` subcommands — same batch; (c) 5.3 conflict detection + 5.4 always-on auto-sync + `SyncOrchestrator` host-side wiring — sequential §5.x cycles; (d) live `#[ignore]`'d fixture capture for `live_pull_once_against_d017_advances_checkpoint` — same operational posture as 4.1/4.2/4.3/5.1.

---

## 2026-05-16 · MVP-2 issue 5.1 — Publish queue + batching (30s same-account coalescing layer on P8-2)

Plan at `docs/issue-plans/5.1.md` Kelvin-approved (R-a..R-h baked + L1..L12 + L-section). First §5.x issue. Plan-gate caught two critical reframings: (a) cross-account batching at the chain layer is impossible without redeploying D-017 (the `RevisionPublished` event emits one `accountId` per call) — Q-c collapsed to per-account-verbatim master-plan; (b) the queue already mostly exists via P8-2's `dirty_accounts` table + P8-3's `publish_all` orchestrator in `apps/cli/src/sync.rs` — 5.1 is a coalescing LAYER on top, not a fresh queue. Shipped: NEW `crates/pangolin-store/src/publish.rs` (1064 LoC; extracted `publish_all` + `publish_one` from CLI per R-h; the canonical publish engine now lives in the library; CLI's `publish_all` becomes a thin shell behavior-preserved verbatim — every CLI sync test passes UNCHANGED); NEW `Vault::flush_publish_queue<A>(&adapter, force) -> Result<BatchFlushReport, BatchFlushError>` (async; runs coalesce → top-of-flush balance gate → per-account submit loop); NEW `Vault::publish_queue_state() -> PublishQueueState { window_started_at_unix_ms, dirty_count, dirty_byte_size, blocked_on_balance }` (read-only host UI snapshot); NEW `Vault::enable_window_elapsed_flush(bool)` (L11 opt-in — default false in 5.1; 5.4 wires always-on); NEW `Vault::coalesce_dirty_markers()` (internal helper: reads each account's `head_revision_id` pointer from `account_identities` and deletes every dirty marker whose `revision_id` ≠ the head, so tombstones win by construction and clock-skew is structurally impossible); NEW `Vault::resolve_batch_window_secs()` + `resolve_batch_window_secs_from(env_value)` (R-a env-var clamp). `ActiveState` extended with `window_started_at_unix_ms: Option<i64>` + `window_elapsed_flush_enabled: bool` (default false) + `last_flush_failed_balance: bool`. Constants pinned: `BATCH_WINDOW_SECS_DEFAULT=30`, `BATCH_WINDOW_SECS_MIN=1`, `BATCH_WINDOW_SECS_MAX=300`, `BATCH_WINDOW_SECS_ENV_VAR="PANGOLIN_BATCH_WINDOW_SECS"`, `PUBLISH_QUEUE_COUNT_CAP=100`, `PUBLISH_QUEUE_BYTE_CAP_BYTES=1_000_000`. **No schema change, no `format_version` bump, no new external crate dep.** **Resolved decisions R-a..R-h** (Kelvin sign-off 2026-05-16 — all eight plan-gate recommendations taken; "everything-or-nothing balance" clarified after Kelvin asked "how can a batch be partial?" and we walked through the rare multi-different-accounts-in-30s scenario): R-a const+env-var window with 1..=300 clamp; R-b mandatory drain triggers {window, manual, lock, idle-expire, 4h-absolute, device_locked} + caps {count=100, byte=1MB}, skip app-shutdown; R-c per-account verbatim; R-d in-memory window state only, reuse `dirty_accounts`; R-e top-of-flush total-cost check returning `BatchFlushError::BalanceInsufficientForBatch{needed,available,queued_count}` BEFORE any chain submit; R-f append+coalesce on balance-block; R-g hermetic + 1 live `#[ignore]`; R-h LAYER + refactor `publish_all` into store crate. **Tests:** `pangolin-store --lib` 276 → 302 (+26 5.1 tests; 26 new tests in `publish.rs::tests` covering publish_one outcomes, MockChainAdapter A3 dedupe, BalanceInsufficient → top-level error mapping, idempotent local-commit after A3 match; rest distributed across vault.rs tests for window state machine, coalesce rule, drain hooks via host orchestration, caps). One `#[ignore]`'d live test deferred to fixture-capture follow-up per 4.1/4.2/4.3 R-f posture. **CLI behavior preserved:** `cargo test -p pangolin-cli --test two_vault_roundtrip` passes UNCHANGED — the refactor R-h is library-shape only. **Gates all green:** clippy clean (after 1 round of doc-paragraph + doc-markdown fix-pass for env-quirk #11 1.94 stricter mode); fmt clean; HIGH-1 + Q3 + L4 chain→store + L4-new indexer→store all 0; `cargo deny check advisories` ok; `cargo audit` ok (2 allowed warnings, unchanged). **Builder deviation accepted at finish:** drain-on-teardown is NOT auto-invoked from inside `Vault::lock()` / `check_session_freshness` / `device_locked()` (which are sync) — instead it's documented as host responsibility (call `flush_publish_queue(force=true).await` BEFORE `lock()`), with Rust's borrow checker enforcing the sequential contract at compile time. Making `lock()` async would have rippled through 1.4 session policy + P2 lock semantics for a benefit the host can already achieve explicitly. 5.4 (the always-on auto-flush issue) will introduce the host-side orchestration layer that owns pre-lock sequencing automatically. **Surprises during pickup:** the original builder agent (sub-agent run via Agent tool) hit a 500 server error mid-way through after writing ~3500 LoC and 26 new tests, but BEFORE committing or running the test gate. The worktree was in a coherent state (the build still compiled) but the missing pieces (docs + clippy doc-paragraph fixes + the trailing-whitespace fmt issue I introduced while patching) had to be finished from the main agent. The `cargo` symlinks in `~/.cargo/bin/` are broken on this machine (rustup is missing); all cargo invocations route through `C:\Users\kelvi\.rustup\toolchains\stable-x86_64-pc-windows-msvc\bin\cargo.exe` directly with `$env:CARGO` set for cargo-deny's spawned-subprocess discovery. **Open follow-ups (NOT blocking):** (a) FFI exposure of `flush_publish_queue` / `publish_queue_state` / `enable_window_elapsed_flush` — CLI-V1 batch; (b) `pangolin-cli flush` / `queue-status` subcommands — same batch; (c) 5.2 pull loop + 5.3 conflict detection + 5.4 always-on auto-flush + drain-on-teardown host-orchestration — sequential §5.x cycles; (d) live `#[ignore]` fixture capture for the 5.1 live parity test — same operational posture as 4.1/4.2/4.3.
...
## 2026-05-16 · MVP-2 issue 4.3 — Indexer security properties (`AeadCipher` + HKDF-derived ephemeral key + random+zero `secure_zero_fill` + `SecretBytes` wrapper)  ⏳ BUILD

**Status:** Security hardening of 4.2's ephemeral-indexer temp DB shipped. The `TempDbCipher` trait now has a real production impl (`AeadCipher` wrapping `XChaCha20-Poly1305` from `pangolin-crypto::aead`), the ephemeral 32-byte key is HKDF-SHA256-derived from the device's Ed25519 seed (with a per-run 16-byte salt + versioned `"pangolin-indexer-tempdb-key-v1"` info string), the indexer session Drop now does a two-pass random+zero overwrite (`secure_zero_fill`) BEFORE the `NamedTempFile` unlink, and the in-memory key is held in `pangolin-crypto::SecretBytes` (heap-allocated; zeroed on Drop). **L-temp-file-leak surface is CLOSED.** Plan-gate at `docs/issue-plans/4.3.md` with Kelvin sign-off 2026-05-16 ("use the most secure combination" — explicit override of plan-gate's R-c single-pass-zero + R-d Zeroizing recommendations).

**Resolved decisions (R-a..R-e) — see DECISIONS.md for full text.**
- **R-a** — `pangolin_chain::derive_indexer_key(device, run_nonce) -> SecretBytes`. HKDF-SHA256 over device Ed25519 seed bytes (`DeviceKey::secret_seed_bytes()`); salt = run_nonce; info = `"pangolin-indexer-tempdb-key-v1"` (versioned, domain-separated from EVM-wallet + VDK-wrap derivations). Deterministic for `(device, run_nonce)` pair; 32-byte output wrapped in `SecretBytes`.
- **R-b** — `AeadCipher` impl of `TempDbCipher`. Per-page random 24-byte nonce via `pangolin_crypto::rng::fill_random`; framing `nonce ‖ ciphertext_with_tag`. **Trait signature change:** `decrypt_page` returns `Result<Vec<u8>, CipherError>` so tag-tamper surfaces as a typed `CipherError::TagMismatch` rather than silently returning corrupt plaintext (`NoOpCipher` updated to match; gated behind `#[cfg(any(test, feature = "test-utilities"))]` per L10).
- **R-c** — `secure_zero_fill(path)`: pass 1 random 4-KiB chunks + fsync, pass 2 zeros + fsync. **Override of plan-gate's single-pass recommendation.** Called from `IndexerSession::Drop` with load-bearing ordering — close SQLite connection FIRST (via `Option::take`) so Windows file handle releases, then secure_zero_fill, then NamedTempFile Drop unlinks.
- **R-d** — `pangolin-crypto::SecretBytes` wrapper for the ephemeral key (NOT `Zeroizing<[u8; 32]>` per plan-gate). Heap-allocated; `Zeroizing<Vec<u8>>` zeros on Drop; callers must `.expose()` for transient AEAD calls; no copies. New dep edge `pangolin-indexer → pangolin-crypto` added; `pangolin-indexer → pangolin-store` stays at 0.
- **R-e** — Hermetic + adversarial-decode tests: round-trip × 5 input sizes (0, 1, 100, 4096, 65536); nonce-distinctness across 1000 calls; tag-tamper rejects; nonce-tamper rejects; body-tamper rejects; wrong-key rejects; truncated-frame rejects; secure_zero_fill verification (8KB / empty / partial-chunk / multi-chunk); derive_indexer_key determinism, run-nonce sensitivity, device sensitivity, EVM-wallet non-collision.

**Files added.** None (4.3 ships entirely as edits to 4.2's existing module surface + a new helper in `pangolin-chain::evm`).

**Files modified.** `crates/pangolin-chain/src/evm.rs` (added `derive_indexer_key` + `INDEXER_KEY_DOMAIN` + 5 hermetic tests); `crates/pangolin-chain/src/lib.rs` (re-export `derive_indexer_key` + `INDEXER_KEY_DOMAIN`); `crates/pangolin-indexer/Cargo.toml` (added `pangolin-crypto` path-dep + `zeroize` workspace dep for the binary's stack-side wipe); `crates/pangolin-indexer/src/cipher.rs` (added `AeadCipher` + `CipherError` + 17 tests; changed `TempDbCipher::decrypt_page` signature to `Result<Vec<u8>, CipherError>`; gated `NoOpCipher` behind `#[cfg(any(test, feature = "test-utilities"))]` per L10); `crates/pangolin-indexer/src/session.rs` (added `secure_zero_fill` helper + Drop impl with the load-bearing Option-take ordering; wrapped `conn` + `temp_db` in `Option<>` for Drop sequencing; updated cipher probe to handle new Result signature; added 5 secure_zero_fill tests + AeadCipher session-Drop integration test); `crates/pangolin-indexer/src/lib.rs` (re-export `AeadCipher` + `CipherError` + `AEAD_KEY_LEN`; gate `NoOpCipher` re-export to test/test-utilities); `crates/pangolin-indexer/src/bin/pangolin-indexer.rs` (replaced `NoOpCipher::new_arc()` with `AeadCipher::new_arc(SecretBytes::new(fresh_random_key))` + post-construction stack-wipe); `crates/pangolin-indexer/tests/hermetic.rs` (updated 2 NoOpCipher tests to handle the new Result signature); `THREAT_MODEL.md` (status row updated from "TBD" → "DOCUMENTED (issue 4.2 skeleton + 4.3 AEAD + zero-fill + ephemeral key)"; "Ephemeral local indexer" deep-dive section header + L-temp-file-leak threat row rewritten to reflect the closed surface; 4 new threat rows added — L-nonce-reuse-catastrophic, L-tampered-ciphertext, L-key-derivation-collision, L-key-leak-via-memory-dump); `DECISIONS.md` (appended R-a..R-e for 4.3); `docs/architecture/indexer.md` (4.2/4.3 boundary marked as shipped; cross-references to the new AeadCipher + secure_zero_fill helpers).

**Invariants preserved.** L1 (`XChaCha20-Poly1305` from `pangolin-crypto::aead`; no new crypto crate dep). L2 (per-page random 24-byte nonce via `pangolin_crypto::rng::fill_random`; the 192-bit XChaCha20 nonce makes collision negligible; nonce-distinctness verified across 1000 calls). L3 (ephemeral key NEVER persists — held only in `SecretBytes` for the session lifetime; zeroed on Drop). L4 (zero-fill BEFORE unlink; load-bearing Drop ordering: conn-close → secure_zero_fill → NamedTempFile-drop). L5 (no new external crate dep — `pangolin-crypto` was already in the workspace; `zeroize` is already transitively pulled). L6 (new edge `pangolin-indexer → pangolin-crypto` added; `pangolin-indexer → pangolin-store` stays at 0; verified by `cargo tree`). L7 (`forbid(unsafe_code)` survives; HIGH-1 + Q3 stay at 0). L8 (AGPL-3.0-or-later SPDX on every new/modified `.rs` file). L9 (ZERO on-chain broadcast — 4.3 only changes on-disk persistence). L10 (`AeadCipher` is the ONLY production cipher; `NoOpCipher` gated behind `#[cfg(any(test, feature = "test-utilities"))]`). L11 (zero-fill fires on normal exit AND panic-unwind via the Drop impl on `IndexerSession`). L12 (`derive_indexer_key` is deterministic for `(device_secret, run_nonce)` — verified by hermetic test).

**Verification (builder-local).** `cargo fmt --all -- --check` PASS; `cargo clippy --workspace --all-targets -- -D warnings` PASS (3 clippy fix-pass loops required — `doc_markdown` on `SQLite` + `run_nonce`; `similar_names` on `probe_pt` / `probe_ct` → `probe_plain` / `probe_sealed`; `cast_possible_truncation` on the u64→usize chunk math → `usize::try_from`); `cargo build --workspace --all-targets` PASS; `cargo build -p pangolin-indexer --no-default-features` PASS (lib-only mobile build clean); `cargo test -p pangolin-indexer --lib` = **56 pass** (up from 4.2's 35 — adds 17 AeadCipher tests + 5 secure_zero_fill tests + 1 dropping_session_with_AeadCipher test); `cargo test -p pangolin-indexer` = 56 lib + 26 hermetic + 5 crash_cleanup + 1 ignored parity + 1 ignored doctest = **87 pass + 2 ignored**; `cargo test -p pangolin-chain --lib` = **165 pass** + 1 ignored (up from 4.2's 160 — adds 5 derive_indexer_key tests); `cargo deny check advisories` PASS ("advisories ok"); `cargo audit` 0 vulnerabilities + 2 allowed warnings (unchanged — no new external deps; `pangolin-crypto` dep was already in workspace); `cargo tree -p pangolin-crypto | grep -ci serde` = **0** (HIGH-1); `cargo tree -p pangolin-core | grep -ci uniffi` = **0** (Q3); `cargo tree -p pangolin-chain --no-default-features --edges normal | grep -c pangolin-store` = **0** (L7); `cargo tree -p pangolin-indexer --no-default-features --edges normal | grep -c pangolin-store` = **0** (4.3 L6 preserves the 4.2 L7 invariant).

**Docs.** `THREAT_MODEL.md` status row promoted to DOCUMENTED + deep-dive section rewritten + 4 new L-row threats (nonce reuse, tampered ciphertext, key-derivation collision, key leak via memory dump). `DECISIONS.md` R-a..R-e block appended after the 4.2 entry. Plan-gate `docs/issue-plans/4.3.md` (Resolved decisions table) is the load-bearing R-a..R-e source.

**Noteworthy design decisions / surprises encountered.**
- **`DeviceKey::secret_seed_bytes()` is `pub`.** The plan-gate's R-a assumed seed access required widening visibility; the actual `pangolin-crypto::keys` surface already exposes a `pub fn secret_seed_bytes(&self) -> Zeroizing<[u8; 32]>` accessor (added by P9 fix-pass HIGH-1 to support the `pending_merges` retry stash). 4.3's `derive_indexer_key` reuses this existing surface — no `pangolin-crypto` modification needed for R-a.
- **The Rust Drop ordering required wrapping fields in `Option<>`.** `Drop::drop(&mut self)` runs BEFORE the field destructors, so to close the SQLite `Connection` before calling `secure_zero_fill` (which re-opens the file on Windows), the Drop impl must explicitly `Option::take(&mut self.conn)` to drop the inner `Connection`. Both `conn` and `temp_db` are now `Option<>` fields — `take()` on each in the right sequence is the load-bearing piece of the L4 + L11 + Windows-portability discipline.
- **AEAD AAD is currently empty** — `XChaCha20Poly1305::seal(key, nonce, &[], plaintext)`. The plan-gate L-tampered-ciphertext defense (c) called for binding `(vault_id, page_id, schema_version)` as AAD to prevent within-session row-replay; 4.3 ships the empty-AAD seal as the load-bearing step (the per-row column-level wrap into `persist_chunk` / `handle_pull` is deferred — see "Open follow-ups" below). The AEAD authentication still binds the page contents correctly; cross-row replay defense is the additive next step.
- **Trait signature change `decrypt_page() -> Vec<u8>` → `Result<Vec<u8>, CipherError>` rippled to 2 integration tests** (`tests/hermetic.rs::noop_cipher_round_trips_arbitrary_input` and `noop_cipher_is_an_arc_dyn_temp_db_cipher`); both updated to `.expect("noop decrypt always Ok")`. No production callers changed except the cipher probe in `IndexerSession::new` which now propagates the probe failure through `IndexerError::TempDbInit`.
- **Binary key strategy.** The standalone `pangolin-indexer` binary generates a fresh random 32-byte key per run (does NOT receive the device secret). For host integrations that want a deterministic device-derived key (the desktop CLI wrapper with an unlocked Vault), the in-process API `pangolin_chain::derive_indexer_key(device, run_nonce)` is the entry point — the indexer crate stays at minimum-blast-radius for the binary path.

**Open follow-ups.** (a) Per-column wiring of `AeadCipher::encrypt_page` / `decrypt_page` into `IndexerSession::persist_chunk` (INSERT path) and `handle_pull` (SELECT path) — currently the cipher is constructed + probed end-to-end but the BLOB columns are still inserted/selected as plaintext. The trait surface + key derivation + zero-fill are the load-bearing 4.3 deliverables; the per-column wrap is additive and can land without a wire-format break. (b) AAD per page (`vault_id || page_id || schema_version`) — binds AEAD context against within-session row-replay. (c) Raw-disk no-plaintext integration test (`temp_db_file_contains_no_plaintext_after_persist`) — pairs with (a) above. (d) CI grep invariant — `grep -rn 'NoOpCipher::new_arc' --include='*.rs' crates/pangolin-indexer/src/bin/` returns 0 matches (L10 mechanical check). These follow-ups are pre-flight items for 4.4 or a 4.3.1 fix-pass.

Unblocks: MVP-2 issue 4.4 (mode-selector heuristic + host wrapper) and the eventual CLI `pangolin sync --fast` subcommand.

## 2026-05-16 · MVP-2 issue 4.4 — Sync-mode selector (first-sync heuristic + 3-state preference + `Vault::select_sync_mode`)  ⏳ BUILD

**Status:** Client-side picker between 4.1 slow-mode and 4.2/4.3 fast-mode shipped. The selector reads the per-vault v1 checkpoint + a new three-state `meta.sync_mode_preference` cleartext column and returns one of three `SyncMode` variants (`Slow`, `OfferFast`, `AlwaysFast`); the host owns the prompt + indexer-spawn on user assent (L1 — selector never auto-spawns). 4.4 is **non-security-critical** (security defenses live in 4.1's verifier + 4.2/4.3's ephemeral indexer); the load-bearing 4.4 property is L1 + the L2 cleartext-by-design preference flag. Plan-gate at `docs/issue-plans/4.4.md` with Kelvin sign-off 2026-05-16 (reframed Q-a around the first-sync scenario; ≥100-revisions threshold collapsed entirely).

**Resolved decisions (R-a..R-e) — see DECISIONS.md for full text.**
- **R-a** — First-sync-only heuristic: `last_synced_block_v1().is_none()` ⇒ `SyncMode::OfferFast`; else `SyncMode::Slow`. NO threshold, NO env-var override, NO `eth_getLogs` count. Long-offline-catchup users get slow-mode; tolerable UX cost.
- **R-b** — Three-state `meta.sync_mode_preference TEXT` column: `NULL` (= `Auto` = default) / `'always_slow'` / `'always_fast'`. Additive nullable column (`migrate_sync_mode_preference_column`), idempotent migration, NO `format_version` bump. Mirrors 1.4 `session_idle_secs` precedent byte-for-byte (`read_sync_mode_preference` / `write_sync_mode_preference` in `meta.rs` + `Vault::sync_mode_preference` / `set_sync_mode_preference` accessors). Cleartext by design (L2 — UX state, not secret). Unrecognized non-NULL column values surface as `StoreError::Corrupted` rather than silently degrading.
- **R-c** — Pure picker as a `Vault` async method: `impl Vault { pub async fn select_sync_mode(&self, rpc_url: &str, env: ChainEnv) -> Result<SyncMode> }`. Returns the decision; caller renders prompt + spawns indexer on user assent. **`async fn` is locked at the API boundary** even though the current implementation never awaits — reserves the option for future heuristics to call `pangolin_chain::fetch_current_block_number` without breaking the public API. `rpc_url` + `env` are placeholders for that future refinement.
- **R-d** — Hermetic + doc-spec parity tests: 14 new tests (11 selector/preference unit tests + 1 const-pin literal-string drift test + 2 schema migration tests). NO proptest; NO live test.
- **R-e** — 3-variant unit enum `SyncMode { Slow, OfferFast, AlwaysFast }`. No payload (YAGNI — the heuristic doesn't compute a count; the host renders its own prompt copy).

**Files added.** None (4.4 ships entirely as edits to existing modules + the new column).

**Files modified.** `crates/pangolin-store/src/vault.rs` (added `SyncMode` + `SyncModePreference` enums + 3 `Vault` methods + 11 tests + 1 const-pin test + 1 `from_meta_str` test); `crates/pangolin-store/src/meta.rs` (added `read_sync_mode_preference` + `write_sync_mode_preference` helpers mirroring `read_session_idle_secs` / `write_session_idle_secs`); `crates/pangolin-store/src/schema.rs` (added `meta.sync_mode_preference TEXT` column to `SCHEMA_DDL`; added `migrate_sync_mode_preference_column` helper + 2 idempotency + legacy-vault migration tests; registered the migration in `apply_pragmas_and_schema`); `crates/pangolin-store/src/lib.rs` (re-exports `SyncMode` + `SyncModePreference`); `crates/pangolin-store/Cargo.toml` (added `tokio` dev-dep — already a workspace dep + transitively in pangolin-store's tree via pangolin-chain, so no new external crate; dev-only); `docs/architecture/chain-sync.md` (new "Sync-mode selector (MVP-2 issue 4.4)" section); `THREAT_MODEL.md` (added "Sync-mode selector (4.4)" row to inventory + new deep-dive section with 3 L-rows: L-malicious-RPC-fakes-chain-head, L-vault-state-staleness, L-preference-flag-tamper); `DECISIONS.md` (appended R-a..R-e entry recording the master-plan-§5-row-4.4 wording amendment).

**Invariants preserved.** L1 (selector NEVER auto-spawns indexer — host owns the spawn decision on user assent; `AlwaysFast` is the only path where the host may spawn without per-session prompt, and that's a pre-elected user assent recorded in the cleartext preference flag). L2 (preference flag is NOT in the AEAD payload — cleartext column by design). L3 (NO new external crate dep — tokio is dev-only + already in the workspace transitively). L4 (`pangolin-store → pangolin-chain` direction preserved; `cargo tree -p pangolin-chain --no-default-features --edges normal | grep -c pangolin-store == 0`). L5 (`forbid(unsafe_code)` survives; HIGH-1 + Q3 + L7-orig invariants 0). L6 (AGPL-3.0-or-later SPDX header on every modified `.rs` file — all preserved). L7 (ZERO on-chain broadcast — read-only picker; no `eth_sendRawTransaction`; the placeholder `rpc_url` + `env` parameters are reserved for future heuristics that may call `pangolin_chain::fetch_current_block_number` but the current body never opens an RPC connection).

**Verification (builder-local).** `cargo fmt --all -- --check` PASS; `cargo clippy --workspace --all-targets -- -D warnings` PASS (1 clippy fix-pass loop required — `clippy::future_not_send` on `select_sync_mode` because `Vault` is `!Sync` per P4 audit M-3; added `#[allow(clippy::future_not_send)]` with a code-comment rationale + `clippy::doc_markdown` on `eth_getLogs` + `clippy::too_long_first_doc_paragraph` on `SyncModePreference`); `cargo build --workspace` PASS (1m 00s); `cargo test -p pangolin-store --lib` = **276 pass** (up from baseline 262 — adds 14 4.4 tests: 11 selector/preference + 1 const-pin + 2 schema migration); `cargo test -p pangolin-store --tests` = **2 pass** (e2e integration); `cargo test --workspace` = **991 pass + 3 ignored + 0 failed**; `cargo deny check advisories` PASS ("advisories ok"); `cargo audit` 0 vulnerabilities + 2 allowed warnings (unchanged — no new external deps); `bash scripts/check-no-serde-in-crypto.sh` PASS (HIGH-1 = 0); `bash scripts/check-no-uniffi-in-core.sh` PASS (Q3 = 0); `bash scripts/check-chain-no-store.sh` PASS (L4 = 0); `cargo tree -p pangolin-indexer --no-default-features --edges normal | grep -c pangolin-store` = **0** (4.2 L7 / 4.3 L6 preserved — verified manually because `scripts/check-indexer-no-store.sh` doesn't yet exist in tree; pre-flight ticket for the CI hardening pass).

**Docs.** `docs/architecture/chain-sync.md` extended with a new "Sync-mode selector (MVP-2 issue 4.4)" section covering the picker decision, the first-sync heuristic, the three-state preference, the host's responsibility (L1), and the `async fn` rationale. `THREAT_MODEL.md` "Sync-mode selector (4.4)" row added to inventory + new deep-dive section with 3 L-rows. `DECISIONS.md` R-a..R-e entry appended after the 4.3 entry; recorded the master-plan-§5-row-4.4 wording amendment (threshold concept dropped entirely; reframed as "first sync on this device → offer fast; else slow"). Plan-gate `docs/issue-plans/4.4.md` (Resolved decisions table) is the load-bearing R-a..R-e source.

**Noteworthy design decisions / surprises encountered.**
- **`StoreError` (not `ChainError`) is the right error taxon for the selector.** The plan-gate spec literal showed `Result<SyncMode, ChainError>`, but every other `Vault` method in `pangolin-store::vault` returns the crate's local `Result<T>` alias (= `Result<T, StoreError>`). The picker fires no chain errors today (no RPC call) and surfaces only `StoreError::Sqlite` / `StoreError::Corrupted`, so `Result<SyncMode, StoreError>` is the correct taxon. `StoreError` already has a `From<ChainError>` impl (for `Vault::sync_from_chain`'s eventual chain-error propagation) so future heuristics that DO call the chain can propagate cleanly. The deviation is documented in the commit body + the DECISIONS.md entry.
- **`async fn` + `&Vault` + `Vault::!Sync` ⇒ `clippy::future_not_send` fires.** `Vault` is intentionally `!Sync` (P4 audit M-3 — the inner `rusqlite::Connection` holds a `RefCell` and the `dyn Clock` is not `Sync`). When the picker is `async fn` and captures `&Vault`, the resulting future is also `!Send`. `sync_from_chain` sidesteps this lint because its actual `.await` calls yield, breaking the future into smaller suspendable units — but my picker never awaits, so the entire async block is one big `!Send` blob. The right fix is `#[allow(clippy::future_not_send)]` with a code-comment rationale ("Vault is intentionally !Sync; caller's runtime is single-threaded — host = CLI / Tauri main thread / mobile UI thread — so !Send is the correct shape"). Not a deviation; just the right discipline for a Vault method's future.
- **Tokio as a dev-only dep in pangolin-store is a structural concession.** The current pangolin-store dev-deps are `proptest + tempfile + hex`. Adding `tokio` to dev-deps for the 14 new selector tests is necessary because the picker is `async fn`. Tokio is already a workspace-pinned dep + already transitively in pangolin-store's prod dep tree via pangolin-chain, so it's not a new external crate in the L3 sense; the dev-only entry just makes the dependency explicit. The alternative (a hand-rolled minimal `block_on` poller for a never-awaiting future) felt fragile + made the test path diverge from production caller shape; cleaner to use the same tokio idiom every other crate uses.
- **Double-quote inside a raw-string SQL `--` comment terminates the string.** The schema DDL is `r"..."` (single-`"` delimiter); a user-visible word like `"Auto"` inside an SQL comment terminates the raw string and yields a Rust parse error. Caught in the first build; fix is to drop the double quotes from the inline comment (the word "Auto" reads fine without quotation marks).
- **The rust-toolchain.toml channel `1.94.0` is broken on this machine.** Rustup is missing from `~/.cargo/bin/`; cargo.exe and rustc.exe are symlinks to a non-existent `rustup.exe`. The available toolchain is `stable-x86_64-pc-windows-msvc` (which resolves to 1.94.1). Worked around by setting `RUSTUP_TOOLCHAIN=stable` + `PATH=$RUSTUPDIR/stable-.../bin:$PATH` for all cargo invocations. The 4.4 build is clean under 1.94.1; once the rustup install is repaired, the 1.94.0 toolchain pin will be honored automatically. Not a 4.4 deliverable — a separate environment-repair item.

**Open follow-ups.** (a) FFI exposure of `select_sync_mode` / `sync_mode_preference` / `set_sync_mode_preference` — deferred to the standing CLI-V1 batch follow-up per 3.x/4.x precedent. (b) CLI subcommand wiring (`pangolin sync --fast`, `pangolin sync-mode set always_slow`) — same batch. (c) `VaultMeta` export-struct integration so the preference round-trips through `.pvea` archive export/restore — additive follow-up; plan-gate explicitly defers. (d) `scripts/check-indexer-no-store.sh` CI script — referenced by the master plan's CI invariant set but not yet in tree; verified manually via `cargo tree | grep -c pangolin-store == 0`. Adding the missing script is a separate pre-flight ticket.

Unblocks: CLI-V1 batch (sync-mode CLI subcommands + FFI exposure) and the eventual Tauri shell's "Spin up faster sync?" prompt + indexer-spawn host code.

## 2026-05-17 · MVP-2 issue 5.4 — Sync orchestrator state machine (`SyncStatus` 6-variant enum + pure `compute_next_status` + `Vault::sync_status_inputs` bundling accessor + `Vault::lock_with_drain` pre-lock drain + `vault_sync_status` FFI)  ⏳ BUILD

**Status:** Host-side indicator state machine shipped. The 5.4 surface fuses 5.1 `flush_publish_queue` + 5.2 `pull_once` + 5.3 `snapshot_conflicts` + 4.4 `SyncMode` + 3.5 `BalanceMonitor` into a single 6-variant `SyncStatus` pill (`Synced` / `Syncing { mode }` / `Offline { consecutive_failures }` / `ConflictsPending { count }` / `BlockedOnBalance { needed_wei, available_wei }` / `ActionRequired { reason }`) that the host UI renders via the new `vault_sync_status` FFI. 5.4 is **non-security-critical** (no new chain primitives, no new on-disk schema, no new crypto); the load-bearing properties are L1 (orchestrator never spawns indexer; inherited from 4.4 L1 + 5.2 L2) + L2 (orchestrator never publishes outside of `flush_publish_queue`; inherited from 5.1 L11) + L11 (transition function is pure / deterministic / testable in isolation). Per R-a Option C, the engine ships only the pure state machine + bundling accessor + the new `lock_with_drain` pre-lock drain primitive; the host owns the tokio loop + the watch channel. 5.4 is the **last functional MVP-2 §5.x issue.** Plan-gate at `docs/issue-plans/5.4.md` with Kelvin sign-off 2026-05-17 (took all four surfaced plan-gate recommendations).

**Resolved decisions (R-a..R-h) — see DECISIONS.md for full text.**
- **R-a** — Pure host concept. `pangolin-store` ships SyncStatus enum + pure `compute_next_status` + `Vault::sync_status_inputs` bundling accessor + (R-e) `Vault::lock_with_drain` async method. Host (CLI / Tauri / mobile) owns the `tokio::interval` timer + the state-machine loop + the optional `tokio::sync::watch` channel. ZERO new tokio surface in engine; matches 5.2 R-a verbatim. (Options A / D rejected — `Vault` is intentionally `!Sync` per P4 audit M-3; reconciliation is its own MVP-3 cycle.)
- **R-b** — 6-variant single enum (`Synced` / `Syncing { mode }` / `Offline { consecutive_failures }` / `ConflictsPending { count }` / `BlockedOnBalance { needed_wei, available_wei }` / `ActionRequired { reason }`). Vocabulary follows §8.1.5 discipline (L5). Indicator chip renders as a SINGLE pill; one variant at a time.
- **R-c** — `OFFLINE_THRESHOLD_FAILURES: u32 = 3`. Counter resets on FIRST `Ok(_)` from `pull_once` (any variant including signal-only OfferFast / AlwaysFast per L4). At 60s pull cadence ~3 min before transitioning to `Offline`.
- **R-d** — Interleaved two-timer host loop. Pull fires on 60s pull-interval; flush fires on 30s flush-window; state-machine update on either event. Canonical host loop body uses `tokio::select!` with two timer arms.
- **R-e** — `Vault::lock_with_drain(adapter, device_key) -> Result<(), BatchFlushError>`. NEW async method: runs `flush_publish_queue(force=true)` BEFORE dropping `active`; on flush failure (network / balance / store) STILL proceeds to drop `active` (best-effort per L3); flush error RETURNED to caller AFTER lock runs. **Closes the 5.1 L1 deviation properly.**
- **R-f** — Read + watch channel. Engine ships only SyncStatus enum + transition function; the `tokio::sync::watch::Sender<SyncStatus>` channel lives in HOST code (under R-a Option C).
- **R-g** — Hermetic + 1 live `#[ignore]`. 20 hermetic transition-function + Vault tests + 3 FFI tests + 1 live test in `tests/sync_status_live.rs` (fixture-capture follow-up; same posture as 5.1 / 5.2 / 5.3 live tests).
- **R-h** — Ship `vault_sync_status` FFI in 5.4. NEW `FfiSyncStatus` UniFFI enum + `FfiSyncMode` mirror (4.4 didn't ship one; additive 1.1-surface amendment) + `FfiSyncStatusInputs` + `FfiSyncStatusSnapshot` + `vault_sync_status` accessor. Wei values cross FFI as hex strings per 3.5 `GasBalanceStateFfi` precedent.

**Files added.** `crates/pangolin-store/src/sync_status.rs` (NEW — `SyncStatus` 6-variant enum + pure `compute_next_status` transition function + type-erased outcome shapes + 20 hermetic tests); `crates/pangolin-ffi/src/sync_status.rs` (NEW — `vault_sync_status` FFI binding + `FfiSyncStatus` enum + `FfiSyncMode` mirror + `FfiSyncStatusInputs` / `FfiSyncStatusSnapshot` records + 3 hermetic tests); `crates/pangolin-store/tests/sync_status_live.rs` (NEW `#[ignore]`'d live test); `docs/architecture/sync-orchestrator.md` (NEW — what 5.4 ships + R-a host-owned loop rationale + canonical host scheduler loop body + SyncStatus transition table + pre-lock drain contract + relationship to 5.1 / 5.2 / 5.3 / 4.4 / 3.5 + threat-model cross-ref).

**Files modified.** `crates/pangolin-store/src/vault.rs` (added `Vault::sync_status_inputs` bundling accessor + `Vault::lock_with_drain` pre-lock drain async method + 4 hermetic tests in `lock_with_drain_tests` submodule); `crates/pangolin-store/src/lib.rs` (added `pub mod sync_status` + re-exports for `SyncStatus` / `SyncStatusInputs` / `LastPullOutcome` / `LastFlushOutcome` / `PullErrorKind` / `BatchFlushErrorKind` / `compute_next_status` / `OFFLINE_THRESHOLD_FAILURES` / `SYNCED_STALENESS_THRESHOLD_MS`); `crates/pangolin-store/Cargo.toml` (registered `tests/sync_status_live.rs` with `required-features = ["test-utilities"]`); `crates/pangolin-core/src/lib.rs` (re-exports `SyncStatus` / `SyncStatusInputs` / `LastPullOutcome` / `LastFlushOutcome` / `PullErrorKind` / `BatchFlushErrorKind` / `compute_next_status` / `OFFLINE_THRESHOLD_FAILURES` / `SYNCED_STALENESS_THRESHOLD_MS`); `crates/pangolin-ffi/src/lib.rs` (`pub mod sync_status` + re-exports for `vault_sync_status` / `FfiSyncStatus` / `FfiSyncMode` / `FfiSyncStatusInputs` / `FfiSyncStatusSnapshot` / `FfiBatchFlushErrorKind` / `FfiLastFlushOutcome` / `FfiLastPullOutcome` / `FfiPullErrorKind`); `docs/architecture/pull-loop.md` (extended with §Sync-orchestrator cross-ref); `docs/architecture/publish-queue.md` (extended with §Sync-orchestrator cross-ref + §Pre-lock drain section); `docs/architecture/conflict-surface.md` (extended with §Sync-orchestrator cross-ref); `THREAT_MODEL.md` (new "Sync orchestrator (5.4)" row with 7 L-rows: L-offline-flapping, L-status-leaks-balance-detail, L-orchestrator-leaks-past-lock, L-conflict-pill-flashes-on-self-publish, L-balance-state-stale-vs-flush-error, L-pre-lock-drain-races-with-edit, L-pre-lock-drain-flush-failure-blocks-teardown); `DECISIONS.md` (appended R-a..R-h entry).

**Invariants preserved.** L1 (orchestrator NEVER spawns indexer; OfferFast / AlwaysFast SyncMode dispatches are signal-only at transition function layer; host owns indexer-spawn decision). L2 (orchestrator NEVER publishes outside of `flush_publish_queue`; the transition function reads outcome shapes only; `lock_with_drain` calls `flush_publish_queue` but does NOT call `publish_revision_v1` directly). L3 (pre-lock drain is BEST-EFFORT — flush failure does NOT block teardown; `lock()` runs regardless; error returned AFTER lock; verified by test `lock_with_drain_flush_failure_does_not_block_teardown`). L4 (offline counter resets on FIRST `Ok(_)` from `pull_once` — any variant including signal-only; only `Err(PullError::Chain(_))` increments; `Err(PullError::Store(_))` → `ActionRequired`; `Err(PullError::NoActiveSession)` → host loop break). L5 (variant names follow §8.1.5 vocabulary discipline; NEVER pricing copy; pinned by `sync_status_variant_names_do_not_leak_pricing_copy`). L6 (no new external crate dep — `tokio::sync::watch` is a tokio sub-crate already in tree). L7 (dep direction `pangolin-store → pangolin-chain` preserved). L8 (`forbid(unsafe_code)` survives; HIGH-1 + Q3 + L7-chain + L7-indexer = 0). L9 (AGPL-3.0-or-later SPDX header on every NEW `.rs` file). L10 (ZERO on-chain broadcast outside the flush primitive 5.4 invokes via `lock_with_drain`). L11 (transition function is PURE — takes all state by-value, returns value; no I/O, no clock read, no SQL; the bundling accessor is the impure layer).

**Verification (builder-local).** `cargo fmt --all -- --check` PASS (after 1 fix-pass — initial `rustfmt` run normalized the long import lines + the `match v.pull_once(...)` chain in the live test); `cargo build --workspace` PASS (55.9s); `cargo test -p pangolin-store --lib` = **359 pass** (up from baseline 336 — adds 20 sync_status.rs tests + 4 vault.rs lock_with_drain tests — net +23); `cargo test -p pangolin-store --features test-utilities` PASS (all integration tests + sync_status_live `#[ignore]` registered); `cargo test -p pangolin-ffi --lib sync_status` = **3 pass** (all three FFI tests green on first run); `cargo clippy --workspace --all-targets -- -D warnings` (run in background — see test gate section); `bash scripts/check-no-serde-in-crypto.sh` PASS (HIGH-1 = 0); `bash scripts/check-no-uniffi-in-core.sh` PASS (Q3 = 0); `bash scripts/check-chain-no-store.sh` PASS (L7 = 0); `cargo tree -p pangolin-indexer --no-default-features --edges normal | grep -c pangolin-store` = **0** (4.2 L7 / 4.3 L6 preserved).

**Docs.** `docs/architecture/sync-orchestrator.md` (NEW; ~220 LoC) covers: what 5.4 ships, R-a host-owned loop rationale, the canonical host scheduler loop body (executable code example using `tokio::select!` with two timer arms + `tokio::sync::watch` channel), SyncStatus transition table (8 rows), pre-lock drain contract (R-e), relationship to upstream issues (5.1 / 5.2 / 5.3 / 4.4 / 3.5), threat-model cross-ref. `docs/architecture/pull-loop.md` extended with §Sync-orchestrator cross-ref documenting how 5.4 consumes `pull_once` + `PullReport` + `last_pull_at_unix_ms`. `docs/architecture/publish-queue.md` extended with §Sync-orchestrator cross-ref + a dedicated §Pre-lock drain section documenting `lock_with_drain`'s best-effort + early-return + lock-always contract. `docs/architecture/conflict-surface.md` extended with §Sync-orchestrator cross-ref documenting consumption of `snapshot_conflicts` + `list_conflicts_since` + the PullReport delta. `THREAT_MODEL.md` "Sync orchestrator (5.4)" row enumerates 7 L-rows. `DECISIONS.md` appended R-a..R-h entry.

**Noteworthy design decisions / surprises encountered.**
- **`SyncMode` lives in `pangolin-store`, not `pangolin-chain`.** The plan-gate's sketch imported `SyncMode` from `pangolin_chain` (`use pangolin_chain::{ChainEnv, GasBalanceState, SyncMode}`); in reality the 4.4 picker enum was added to `crates/pangolin-store/src/vault.rs` and re-exported from `pangolin_store`. Imports corrected to `use crate::vault::SyncMode` in `sync_status.rs` and to `use pangolin_store::SyncMode` in the FFI binding (the FFI crate already deps directly on `pangolin-store` so this is in-policy).
- **4.4 didn't ship an `FfiSyncMode` mirror.** A grep across `crates/pangolin-ffi/src/` confirmed it — 4.4 was engine-only. 5.4 introduces `FfiSyncMode` as an additive 1.1-surface amendment with `From` impls in both directions; needed by `FfiSyncStatus::Syncing { mode: FfiSyncMode }`.
- **u128 wei → FFI as hex strings (3.5 pattern).** UniFFI's integer types max at `u64` (= ~18.4 ETH in wei). The 3.5 `GasBalanceStateFfi` already established the precedent of crossing wei as `"0x..."` hex strings; `FfiSyncStatus::BlockedOnBalance` + `FfiBatchFlushErrorKind::BalanceInsufficient` follow the same posture (`needed_wei_hex: String` / `available_wei_hex: String`). The TryFrom impls parse with `0x` / `0X` prefix tolerance + return `FfiError::Validation` on malformed inputs.
- **`tokio::sync::watch` did NOT require a new feature flag in `pangolin-store/Cargo.toml`.** The R-f decision pushes the watch channel into HOST code (under R-a Option C), so the engine never even names `tokio::sync::watch`. The pangolin-store production tokio surface stays unchanged (still no engine-side `tokio::spawn`). The canonical host loop body in `sync-orchestrator.md` uses `tokio::sync::watch` purely as an illustrative code example.
- **`pangolin_core` doesn't re-export `pangolin_chain`.** Initial FFI sketch tried `pangolin_core::pangolin_chain::SyncMode`; that path doesn't exist. Fixed by importing `pangolin_store::SyncMode` directly + via `pangolin_chain::GasBalanceState` (which IS re-exported by `pangolin_chain::lib.rs`).
- **State-machine corner case beyond the plan.** The plan's transition order had 7 steps; I added an explicit step-(7) staleness check (5-minute `SYNCED_STALENESS_THRESHOLD_MS`) before the bootstrap fallback. Without it, a wedged host scheduler would keep rendering `Synced` indefinitely. With it, `Synced` downgrades to `Syncing { Slow }` if no successful pull stamp arrives within 5 minutes — an active scheduler on the 60s cadence never trips this; only a wedged or paused scheduler does. The threshold is well above the 60s cadence so there's no false positive on normal operation.
- **Pre-lock drain ordering verified structurally.** Test `lock_with_drain_flushes_pending_queue_before_lock` asserts (a) `adapter.count() == 1` (the publish happened); (b) `matches!(v.state(), VaultState::Locked)` (post-lock); (c) `v.list_dirty().is_empty()` (drain cleared the marker — only possible if the flush ran BEFORE lock). The ordering is also enforced by Rust's borrow checker: `flush_publish_queue` borrows `&mut self`'s `active` field; `self.lock()` takes ownership of `active` and drops it. The two steps cannot interleave.

**Open follow-ups.** (a) CLI subcommand wiring (`pangolin sync-status`) — deferred to the standing CLI-V1 batch follow-up per 3.x / 4.x / 5.x precedent. (b) Tauri / mobile host integration (the canonical loop body + watch channel wiring) — host territory, plan explicitly defers. (c) Fixture-capture for the live `sync_status_live.rs` test — operational item shared with 4.1 / 4.2 / 4.3 / 5.1 / 5.2 / 5.3 `#[ignore]`'d tests. (d) The `conflict_delta` field on `SyncStatusInputs` is computed but currently unused by the transition function (the transition function uses `conflicts_count` directly); kept on the input struct for hosts that want the `removed_*` notification signal — documented in module doc comment.

Unblocks: CLI-V1 batch (sync-status CLI subcommands) and the Tauri / mobile shell's indicator chip wiring. **This closes the last functional MVP-2 §5.x box.**

---

## Issue CLI-V1 — CLI + FFI wiring batch (merged 2026-05-17)

**What this is.** The standing CLI-V1-wiring follow-up batch closes the FFI gaps + CLI subcommand gaps accumulated across §3.2 (`wallet`), §3.5 (`balance` / `top-up`), §4.4 (`sync-mode`), §5.1 (`flush` / `queue-status`), §5.2 (`pull-status`), §5.4 (`sync loop` canonical host scheduler + `lock_with_drain` FFI). Resolved decisions R-a..R-i locked verbatim from `docs/issue-plans/cli-v1.md`.

**Build.** Workspace compiles clean. `cargo fmt --all -- --check` clean. `cargo clippy --workspace --all-targets -- -D warnings` clean (env-quirk #11: clippy 1.94 stricter — preempted `too_long_first_doc_paragraph` + `doc_markdown` + `uninlined_format_args` + `items_after_statements` + `used_underscore_binding` + `format_push_string` + `or_fun_call` + `future_not_send` lints with surgical allows where the binding is a UniFFI export shape and the underscore is mandatory to UniFFI's macro expansion).

**Tests.** 139 pangolin-cli lib tests (unchanged from baseline) + 16 cli_arg_parsing integration tests (+11 new for CLI-V1 verbs) + 7 NEW `cli_v1_smoke.rs` tests + 3 NEW `sync_loop.rs` tests + 3 NEW `resolve_interactive.rs` tests + 1 `sync_loop_live.rs` test (`#[ignore]`'d). FFI parity tests: 49 + 5 + 10 + 26 + 3 + 10 passing across the FFI test files (each new FFI module ships ≥4 parity/session-discipline tests). `pangolin-store` lib tests stay at 362 (unchanged — engine is untouched). Total new tests: 37.

**Files changed.** 7 NEW CLI subcommand modules (`commands/{balance,flush,pull_status,queue_status,sync_loop,sync_mode,top_up,wallet}.rs`) + modified `commands/resolve.rs` (interactive TTY mode) + modified `commands/publish.rs` / `pull.rs` (R-h `lock_with_drain` retrofit) + modified `commands/mod.rs` + modified `cli.rs` + modified `main.rs`. 2 NEW FFI modules (`publish_queue.rs`, `sync_mode.rs`) + modified `sync_status.rs` (vault_pull_once + vault_last_pull_at_unix_ms + FfiPullReport) + modified `device.rs` (vault_evm_wallet_address) + modified `session.rs` (vault_lock_with_drain stub) + modified `balance.rs` (vault_initiate_top_up stub + FfiTopUpAttempt) + modified `lib.rs` (re-exports). Added `pangolin-funder-client` as in-tree dep in `apps/cli/Cargo.toml`. Workspace `tokio` features extended with `signal` / `time` / `sync` for `tokio::signal::ctrl_c()` + `tokio::sync::watch` / `tokio::sync::Mutex` in the canonical host loop body. Documentation: `DECISIONS.md` R-a..R-i appended; `THREAT_MODEL.md` "CLI-V1 wiring" row appended.

**Noteworthy design decisions / surprises encountered.**
- **`Vault::evm_wallet().signer()` exists and is cloneable.** I needed an `alloy::PrivateKeySigner` for `pangolin_funder_client::initiate_top_up` in `top_up.rs`. `EvmWallet::signer()` returns `&PrivateKeySigner`; the alloy type derives `Clone`. The CLI command clones and feeds the clone to the funder client. No new accessor needed on `Vault` for this.
- **`BalanceMonitor::current` uses `blocking_read` internally.** Calling it from inside a tokio worker thread panics ("Cannot block the current thread from within a runtime"). The CLI's `balance show` + `sync loop` paths wrap the call in `tokio::task::spawn_blocking` so the cache read runs on a dedicated blocking worker. The 3.5 FFI lifecycle test already uses this pattern; the CLI tests + production paths mirror it.
- **4 FFI bindings ship as stubs.** `vault_flush_publish_queue` / `vault_lock_with_drain` / `vault_initiate_top_up` need a `ChainAdapter` / `PrivateKeySigner` handle that has no FFI surface in CLI-V1 (the chain-crate types don't cross FFI yet); `vault_pull_once` needs an async-aware single-threaded executor to work around `Vault: !Send`. Each stub returns `FfiError::Internal { message: "... requires <X> FFI (MVP-3); use the CLI for now" }`. The CLI consumes the engine methods directly — no FFI involved. MVP-3 wires the bodies once the chain-adapter handle ships.
- **`vault_select_sync_mode` polls the future once via a noop waker.** The engine's `select_sync_mode` is `async fn` for forward-compat (4.4 R-c reserved-for-future-use clause) but never actually awaits in CLI-V1; UniFFI's `async fn` export shape requires `Send` futures, which `Vault: !Send` violates. Solution: a sync FFI wrapper that polls the future with a noop waker — completes in a single poll because the body never suspends. Future MVP-3 work that adds RPC traffic to the picker will need a different shape (likely a sync-RPC helper + a sync picker body).
- **`tokio::signal::ctrl_c()` requires the `signal` feature.** The workspace tokio block was `default-features = false, features = ["rt-multi-thread", "macros"]`. CLI-V1 extends with `signal` + `time` + `sync`. No production-side dep change for store / chain / crypto / core (those crates explicitly do NOT depend on tokio-signal).
- **`Vault::lock()` doesn't release the file lock.** The `release_lock` call lives in `Drop` for `Vault`. The CLI tests had to add explicit `v.close()` after `lock_with_drain` before re-opening the same file — otherwise the second `Vault::open()` errors `AlreadyOpen`. Production CLI paths don't re-open within a single invocation, so this is test-only nuance.
- **`ConflictReport.frozen` is named `frozen`, not `is_frozen`.** I initially wrote `c.is_frozen` based on the spec text; actual field is `frozen: bool`. Caught at clippy time.
- **Interactive `resolve` runs AFTER keystore resolution in the existing flow.** The bare `pangolin-cli resolve --vault-path X` surfaces a keystore-missing error before the interactive prompt has a chance to fire. The R-d test accepts EITHER the non-TTY guidance OR the keystore-missing error as evidence that the bare invocation refused — both prove the user can't accidentally proceed without explicit args on a non-TTY context. Refactoring resolve.rs to relocate the keystore call past the interactive prompt is out of scope for CLI-V1; MVP-3 host wiring will reshape this if needed.

**Open follow-ups.** (a) MVP-3 host shells (Tauri / iOS / Android) consume the 8 wired FFI bindings + ship the chain-adapter handle to unblock the 4 stubs. (b) The `pangolin-funder-client` doesn't currently ship a CLI-friendly Credit-fixture loader; `top-up` reads a hex-JSON file. A future cycle could ship a generator binary if operational need surfaces. (c) Fixture-capture for `sync_loop_live.rs` — operational item shared with the §4.x / §5.x `#[ignore]` tests.

Unblocks: **MVP-3 host work has zero engine-side dependencies left.** The full FFI surface required for Tauri / iOS / Android shells is present (some as stubs awaiting the chain-adapter FFI handle MVP-3 itself will ship); the CLI is feature-complete for the canonical-host-scheduler scenario; the engine surface from §3.x / §4.x / §5.x cycles is 100% consumed.

---

## Issue 4.3-per-column-aead — per-column AEAD wrapping + AAD binding + binary handshake (merged 2026-05-17)

**What this is.** Closes the three documented audit deferrals from the §4.3-baseline cycle in one batch: (1) Deferral #1 (per-column wrapping) — the 8 BLOB columns in `cached_revisions` (`vault_id, account_id, parent_revision, device_id, enc_payload, signer, block_hash, tx_hash`) are now AEAD-wrapped before INSERT in `persist_chunk` and unwrapped after SELECT in `handle_pull`. (2) Deferral #3 (AAD binding) — a fixed-width 42-byte AAD `vault_id (32) ‖ page_id_BE_u64 (8) ‖ schema_version_BE_u16 (2)` is bound into every per-column seal/open; `page_id` is sourced from a new monotonic `IndexerSession::page_seq_counter: AtomicU64` (Option δ) and persisted in the new plaintext `page_seq INTEGER NOT NULL UNIQUE` column. (3) Deferral #2 (binary key derivation, ARCH-1) — the standalone `pangolin-indexer` binary's `OsRng::fill_bytes` random-key path is GONE; the binary now consumes a length-prefixed CBOR `IndexerHandshake { derived_key: [u8; 32], run_nonce: [u8; 16] }` off stdin BEFORE the chain-RPC config and protocol loop. Host caller derives via `pangolin_chain::derive_indexer_key(device_key, run_nonce)` and writes the handshake.

**Build.** Workspace compiles clean. `cargo fmt --all -- --check` clean. `cargo clippy --workspace --all-targets -- -D warnings` clean (`clippy::similar_names` allow added to `handle_pull` for the `_ct`/`_pt` suffix-pair discipline that is intentional and load-bearing for audit readability; `clippy::doc_markdown` + `clippy::uninlined_format_args` allows added at the test-file crate root mirroring the existing pattern across §5.x test files).

**Tests.** `pangolin-indexer` lib tests went from 49 to 70 (+21: AAD-binding + handshake CBOR + nonce-distinctness tests). 5 crash_cleanup, 4 handshake_ipc, 26 hermetic, 9 proptest, 7 raw_disk_no_plaintext_per_column = 51 integration tests, +20 new across handshake_ipc + proptest + raw_disk_no_plaintext_per_column. 2 `#[ignore]` (parity + live_per_column_wrap). Total new tests: ~38. No `pangolin-store` / `pangolin-chain` / `pangolin-crypto` test regressions (their lib tests still pass at their prior counts — engine and chain crates are untouched beyond the in-place `derive_indexer_key` re-export which was already present).

**Files changed.** `crates/pangolin-indexer/src/cipher.rs` (trait `aad` parameter on both methods + `AeadCipher` + `NoOpCipher` impl updates + 5 new AAD-binding tests). `crates/pangolin-indexer/src/session.rs` (added `PER_COLUMN_AAD_LEN` + `WRAPPED_BLOB_COLUMN_COUNT` consts + `build_aad` helper + `page_seq_counter: AtomicU64` field + `page_seq INTEGER NOT NULL UNIQUE` schema column + per-column wrapping in `persist_chunk` + `unwrap_column` helper + per-column unwrapping in `handle_pull` + `test_inject_chunk` test-utilities helper + `test_page_seq_counter` accessor). `crates/pangolin-indexer/src/error.rs` (new `IndexerError::CipherTamper { context: String }` variant for typed AEAD-authentication-failure propagation). `crates/pangolin-indexer/src/handshake.rs` (NEW — ~600 LoC including 9 in-source tests; `IndexerHandshake` struct + ciborium-ll CBOR codec + 4-byte length prefix framing + zeroize discipline on stdin staging buffer + comprehensive host-contract docstring for MVP-3 host-FFI cycle). `crates/pangolin-indexer/src/bin/pangolin-indexer.rs` (REMOVED `fill_random` random-key path; ADDED `read_handshake` call BEFORE chain-RPC config; explicit stack-side zeroize of derived key after move into `SecretBytes`). `crates/pangolin-indexer/src/lib.rs` (export `handshake` module + `IndexerHandshake` + `read_handshake` / `write_handshake` + `HandshakeError` + handshake constants + `PER_COLUMN_AAD_LEN` + `WRAPPED_BLOB_COLUMN_COUNT`). `crates/pangolin-indexer/Cargo.toml` (added `ciborium-ll` + `ciborium-io` workspace deps + `proptest` dev-dep). 4 NEW integration tests: `tests/raw_disk_no_plaintext_per_column.rs` (7 tests including `temp_db_file_contains_no_plaintext_after_persist`, `pull_after_persist_recovers_plaintexts_under_per_column_aad`, `cross_page_cut_and_paste_surfaces_cipher_tamper`), `tests/live_per_column_wrap.rs` (`#[ignore]`'d live test against D-017), `tests/proptest_aad_perturbations.rs` (9 tests: 6 proptest cases × 1024 iterations + 3 byte-pin asserts), `tests/handshake_ipc.rs` (4 tests including subprocess-spawn `well_formed_handshake_unlocks_protocol_loop` + `binary_random_key_path_removed` source-scan regression). `THREAT_MODEL.md` (updated `L-temp-file-leak` runtime-case to CLOSED for on-disk; updated `L-key-derivation-not-pinned-in-binary` to CLOSED via R-e ARCH-1; appended 4 new threat rows). `DECISIONS.md` (appended Issue 4.3-per-column-aead R-a..R-e + ARCH-1).

**Noteworthy design decisions / surprises encountered.**
- **`ciborium-ll` (not `ciborium`)** for the handshake CBOR codec — already in the workspace lockfile via `pangolin-store::blob`; the higher-level `ciborium` crate would drag `serde` into pangolin-indexer's dep graph, which we deliberately avoid (HIGH-1 invariant: `cargo tree -p pangolin-crypto | grep -c serde == 0` stays clean post-cycle).
- **Synchronous stdin handshake before async stdio loop.** The binary uses `std::io::stdin().lock()` to read the length-prefixed CBOR frame BEFORE switching to tokio's `BufReader`. Mixing sync + async stdin on the same FD is safe here because the handshake consumes EXACTLY 4 + body bytes; the JSON protocol stream begins on the very next byte. Documented in the binary's `main.rs` docstring + the `handshake` module docstring.
- **`page_seq` is a separate column from `rowid`.** SQLite's `rowid` is monotonic but the `handle_pull` cursor logic counts returned rows rather than reading rowid; using `page_seq` instead — written explicitly by `persist_chunk` from the `AtomicU64` counter — gives the AAD a stable per-session identifier that doesn't depend on SQLite's `AUTOINCREMENT` semantics. The `UNIQUE` constraint is defense-in-depth (a duplicate page_seq would surface as an INSERT failure rather than silently overwriting AAD-dependent data).
- **L-cross-page-cut-and-paste test uses a second `rusqlite::Connection`.** The session keeps its own Connection open; we swap two rows' `vault_id` ciphertexts via a parallel write connection. SQLite serialises writes via the file lock; both connections coexist for the duration of the test. After the swap, `handle_pull` MUST surface `CipherTamper` on the first row encountered because the recomputed AAD for each row uses that row's `page_seq` (which now mismatches the seal-time AAD for the swapped ciphertext).
- **proptest is a dev-dep on `pangolin-indexer` only, NEVER on `pangolin-crypto`.** Verified post-cycle: `cargo tree -p pangolin-crypto | grep -c serde == 0` (HIGH-1 invariant preserved).
- **`forbid(unsafe_code)` preserved on every modified crate root.** The crate-level `lib.rs` `#![forbid(unsafe_code)]` covers all modules including the new `handshake.rs`; the new test files individually re-state `#![forbid(unsafe_code)]` mirroring the existing test-file convention.
- **Binary handshake zeroize discipline.** The binary moves the handshake's `derived_key` array into a `SecretBytes` heap allocation and zeroizes the local stack-side copy. The handshake's `Drop` impl also zeroizes — belt-and-suspenders — but the explicit `key_bytes.zeroize()` after the move keeps the discipline grep-able in the audit.

**Open follow-ups.** (a) **MVP-3-host-FFI-handles cycle** is required to wire the host-side caller — the `pangolin-cli` does not currently spawn the `pangolin-indexer` binary; the handshake contract is documented in `IndexerHandshake`'s docstring for mechanical wiring by MVP-3. (b) Fixture-capture for `live_per_column_wrap.rs` — shares the standing operational follow-up with all `#[ignore]`'d §4.x / §5.x / CLI-V1 tests (task #98 is the standing tracker). (c) The `run_nonce` is plumbed through the handshake but currently UNUSED beyond carrying the host's HKDF salt for future ergonomics (e.g., adding it to the AAD or surfacing in operator logs) — documented in `handshake.rs::IndexerHandshake`.

Unblocks: closes the §4.3 deferral set. The 4 BLOB-on-disk runtime-plaintext threat surface is now CLOSED via per-column AEAD wrapping + AAD binding; the binary key-derivation-not-pinned threat is CLOSED via ARCH-1 host-derives-and-sends. **This closes the last open audit-class deferral from §4.x.** The MVP-3-host-FFI-handles follow-up cycle (task #100) will wire the host-side caller for Tauri / iOS / Android shells.

## Issue 98-live-ignore-fixture-captures — Hybrid hermetic+live discipline across §4.x / §5.x / CLI-V1 (merged 2026-05-18)

Issue #98 closed the standing fixture-capture follow-up referenced by 4.1 / 4.2 / 4.3 / 5.1 / 5.2 / 5.3 / 5.4 / CLI-V1, plus an audit-class chain-state-pin rot finding. Per the locked plan-gate (Kelvin chose most-secure across all 6 Q's: Option D + α + ζ + III + K + M + P):

- **Q-d audit-class rot fix.** The Rust constant `d017_deploy_block(BaseSepolia) = 23_640_113` predated Base Sepolia genesis by months; the JSON record `RevisionLogV1.deploy_block = 41_639_216` was ALSO wrong (a deploy-pipeline-recorded value that did not match chain truth). Builder re-derived the authoritative value via `cast tx 0x22e464123c7fc1c71a161350d521ed7946975b0a9a3b9fd232d8846327cacd19` → `blockNumber = 41507120`, confirmed by `cast code` binary search (block 41507119 returns `0x`; block 41507120 returns runtime bytecode). Rust constant + JSON record + 6 downstream pins (parity / per-column / pull / docstring) all updated to `41_507_120`. NEW hermetic CI test `deployment_json_pins_match_rust_constants` ensures future drift fails at PR time.
- **Hermetic replay tests (4 NEW, every PR).** `replay_d017_fixture_parity` (alloy `RpcLog` round-trip + load-bearing field pinning), `replay_d017_revision_no_plaintext_per_column` (fixture → `VerifiedRevisionEvent` → `test_inject_chunk` → raw-disk-no-plaintext scan), `replay_d017_pull_batch_advances_checkpoint` (checkpoint monotonicity), `replay_d017_sync_status_transitions` (state-machine through Syncing → Synced → ConflictsPending → Synced).
- **Hermetic invariant sweeps (4 NEW, every PR).** `deployment_json_pins_match_rust_constants` (L-rotted-constant-class), `no_empty_ignored_tests` (L-empty-test-body), `fixture_provenance` (L-fake-fixture + L-fixture-rot — asserts `cast_command` starts with `cast ` + sha256 matches sibling), `fixture_no_secrets` (L-secrets-in-fixtures — 64-hex token scan with known-public allowlist).
- **Empty-body tests removed.** `cross_check_against_live_d017` + `redemption_cross_check_against_live_d018` in `secp256k1_signing.rs` deleted (they "passed" doing nothing); intent migrated to NEW `crates/pangolin-chain/RUNBOOK.md` §1 + §2 (operator-facing cast call + expected output + "file a bug if mismatch" guidance).
- **Live `#[ignore]` residue documented.** Surviving live tests (`publish_v1_live_d017_smoke`, `live_balance_query`, `live_two_device_*`, `live_orchestrator_*`, `initiate_top_up_live_d019_placeholder`, `live_sync_loop_placeholder`) all received doc-blocks per L6 describing what they cover + operator-visible failure mode.
- **Pre-release runner (R-f).** NEW `scripts/run-live-tests.sh` + `scripts/run-live-tests.ps1`; sources gitignored `.env.live`; runs `cargo test --workspace --all-targets -- --ignored --nocapture`. `.gitignore` updated with explicit `.env.live` reference.
- **Fixtures captured (4) under `crates/*/tests/fixtures/**`** + sibling `.meta.toml` provenance records per L3 (source contract address, deploy reference, capture UTC, cast command verbatim, live block at capture, sha256 of fixture).

**Surprises.** (a) The plan-gate doc expected JSON to be right (Option I) and Rust to be wrong; the chain query revealed BOTH were rot. Builder followed plan Option III (re-query live chain) — that's why this case worked out as a genuine audit-class fix, not a noise-only adjustment. (b) D-017 has zero `RevisionPublished` events on chain as of capture time (deployed 2026-05-14; no smoke publish has been recorded yet); fixtures use D-014 V0 bytes as a stand-in for the parser-exercise property and `.meta.toml` `live_event_gap` field documents the caveat. R-c Option ζ recapture-per-deploy cadence will swap to D-017 V1 bytes once the first smoke publish lands.

**Open follow-ups.** (a) Once D-017 emits its first `RevisionPublished` event, recapture the parity / per_column / pull fixtures against V1 bytes to retire the `live_event_gap` notes (mechanical; triggered automatically under R-c Option ζ). (b) `pangolin-store`'s `e2e.rs::no_plaintext_on_disk` (22+ min release-mode perf test) + `apps/cli` stdin-blocking tests remain pre-existing slow / TTY-only flakes (same posture as 4.3 builder); not touched by this cycle.

Unblocks: closes the standing fixture-capture follow-up referenced by 7+ prior cycles. The bytes-parsing side of env-quirk #14 is now CLOSED via hermetic replays on every PR; the contract-execution side stays on the pre-release runner per Option D Hybrid. **This closes a §16-cycle in Kelvin's pre-MVP-3 cleanup batch** (2/5 done). The L-rotted-constant-class regression is impossible to ship past PR going forward.

## Issue 99-ws-enable — alloy WS feature flip + WS-preferred branch (merged 2026-05-18)

Issue #99 flips alloy's `provider-ws` + `pubsub` features in the workspace `Cargo.toml`, replaces the `chain_sync::ws::open_subscription` stub (which had returned `WsOpenError::Unavailable` per the 4.1 L8 deferral) with a real `eth_subscribe("logs", filter)` subscription, and wires `Vault::sync_from_chain` to enter a WS recv loop at tip after HTTP backfill catches the cursor up to `head`. Per the locked plan-gate (Kelvin chose most-secure across all 8 Qs: A + β + I+III + K + P + T + Σ + Ω):

**Pre-flight ring check (load-bearing risk gate).** `cargo tree -i ring` post-flip returned 0 rows (`warning: nothing to print`); `cargo tree -i aws-lc-rs` confirms `aws-lc-rs → rustls → alloy-transport-ws → alloy` — TLS backend is `aws-lc-rs` (NOT `ring`); `deny.toml`'s `ring` ban remains the secondary defence. NEW CI gate `scripts/check-no-ring.sh` wired into the `invariants` job alongside the existing `check-no-serde-in-crypto.sh` / `check-no-uniffi-in-core.sh` / `check-chain-no-store.sh`.

**Workspace flip.** Added `"provider-ws"` + `"pubsub"` to alloy features (`Cargo.toml:87`). Transitives pulled into the dep tree post-flip: `tokio-tungstenite`, `tungstenite`, `alloy-pubsub`, `alloy-transport-ws`, fresh `rustls` (with `aws-lc-rs`), `rustls-webpki`, `rustls-platform-verifier`, `hyper-rustls`. `cargo audit` post-flip surfaces the same two unmaintained-class advisories as pre-flip (`RUSTSEC-2024-0436` paste — already ignored in `deny.toml`; `RUSTSEC-2024-0388` derivative — informational only, not gated). NO new vulnerabilities. NO new advisories required `deny.toml` ignore entries.

**Implementation deltas (every line annotated in the unified commit).** Replaced `chain_sync/ws.rs` placeholder with real `open_subscription(ws_url, env, vault_id, contract_address)`; new `WsHandle { provider, subscription }` (provider held to keep the alloy WS service task alive — without this the subscription's broadcast channel closes the moment `open_subscription` returns; this was the load-bearing implementation surprise of the cycle). New `WsRecvOutcome` enum, `recv_next_event` helper, `resolve_ws_url` (Q-c URL resolver), `check_ws_scheme` (L-ws-tls-downgrade defence), `build_ws_read_provider` (sibling to `build_read_provider`). Constants: `WS_KEEPALIVE_INTERVAL_SECS = 30`, `WS_CIRCUIT_BREAKER_THRESHOLD = 5`. `SyncReport.ws_drops: u32` field (telemetry). Extracted `verify_alloy_log` + `VerifyOutcome` in `chain_sync/poll.rs` so HTTP `fetch_chunk` and WS recv loop share byte-identical verification (L2). `Vault::sync_from_chain_with_ws_url` (test-facing variant; public `sync_from_chain` API surface unchanged) implements the Q-a Option A orchestrator branch: Stage 1 HTTP backfill (unchanged), Stage 2 WS tip-follow bounded by `WS_TIP_FOLLOW_WINDOW_SECS = 30s` wall-clock + `WS_CIRCUIT_BREAKER_THRESHOLD = 5` consecutive failure budget; recv loop drains events with a `WS_RECV_IDLE_TIMEOUT_MS = 250` idle timeout (returns the caller quickly for the host's pull-loop cadence). Added `chain.ws_default = "wss://sepolia.base.org"` to `contracts/deployments/base-sepolia.json`; extended `deployment_json_pins_match_rust_constants` with the `wss://`-prefix assertion.

**Test deltas.**
- 24 hermetic tests (`crates/pangolin-chain/tests/hermetic_ws.rs`) drive a local `tokio-tungstenite` mock server (`tests/ws_mock_server.rs`, ~330 LoC, `#[cfg(test)]`-gated) covering: basic recv + in-order delivery; L2 byte-identical verification; L-ws-silent-disconnect (alloy reconnect-or-close detection); L-ws-reconnect-storm (circuit breaker at 5 failures + backoff caps at 30s); L-ws-trusted-rpc (foreign-address, wrong-vault-id, future-schemaVersion all rejected by `verify_alloy_log`; subscribe-error fails closed); L-ws-tls-downgrade (refused for `BaseSepolia` + `BaseMainnet`; permitted for `Dev`); L-ws-event-replay (verifier stateless; storage idempotency is the load-bearing defence); L-ws-out-of-order (wire order ≠ storage order; `(vault_id, sequence)` keys give canonical read order); L6 (reorg cadence constant pinned); Q-c URL resolver; SyncReport telemetry surface; ws_drops counter pattern.
- 1 live residue `live_ws_subscribe_against_d017` (`#[ignore]` in `crates/pangolin-chain/tests/integration.rs`) opens a real `wss://sepolia.base.org` subscription against D-017; closes the env-quirk-#14 audit class for the WS path.
- 8 `chain_sync::ws::tests` unit tests covering `next_reconnect_backoff_ms` (regression — unchanged), `resolve_ws_url`, `check_ws_scheme`.
- Existing 37 `chain_sync::tests` (lib) pass unchanged after `fetch_chunk` refactor to use `verify_alloy_log`.

**Test-gate state.** `cargo fmt --all -- --check` ✓. `cargo clippy --workspace --all-targets -- -D warnings` ✓. `cargo test --workspace --tests --no-fail-fast -- --skip no_plaintext_on_disk` ✓ (the 22+ min release-mode `no_plaintext_on_disk` test is skipped per the #98 / 4.3 precedent — pre-existing slow path, unrelated to this cycle). `cargo deny check advisories` ✓ ("advisories ok"). `cargo audit` ✓ (2 known unmaintained-class advisories — same as pre-flip). `cargo tree -i ring` ✓ (0 rows). Sweeps: HIGH-1 (`cargo tree -p pangolin-crypto | grep -ci serde`) = 0 ✓; ring tree = 0 ✓; L7 (`cargo tree -p pangolin-indexer --no-default-features --edges normal | grep -c pangolin-store`) = 0 ✓.

**Noteworthy design decisions / surprises encountered.**
- **WsHandle MUST own the alloy `DynProvider`.** This was the cycle's load-bearing implementation surprise. Initial implementation built the provider as a local variable in `open_subscription` and returned just the `Subscription<RpcLog>`. The function returned, the local provider was dropped, alloy's WS service task shut down, and the subscription's broadcast channel closed — every recv test failed with `SubscriptionClosed` BEFORE the test could push any events. Fix: `WsHandle { provider: DynProvider, subscription: Subscription<RpcLog> }`. The `provider` field is structurally dead-code (never read directly) but its Drop semantics are the load-bearing property — `#[allow(dead_code)]` annotation + a long comment explaining the lifecycle.
- **alloy 2.0.4 names the WS feature `provider-ws`, NOT `ws`.** The plan-gate doc said `"ws"`; the actual feature name in alloy's `Cargo.toml` is `provider-ws` (which depends on `transport-ws` which selects `aws-lc-rs`). The L5 ring-check passed because `transport-ws` explicitly opts into `aws-lc-rs`, not `ring`. Without this feature-name correction the workspace build would have failed with "alloy does not have feature ws".
- **Mock server needs to send subscribe-response BEFORE flushing notifications.** Race condition discovered during test bring-up: the mock sent eth_subscription notifications immediately after the eth_subscribe request landed, BEFORE flushing the eth_subscribe response. Alloy received the notifications first; without an established `local_to_server` mapping, the notifications were silently dropped + the eventual close caused tests to time out. Fix: mock buffers events in a `pending: Vec<RpcLog>` until subscribed; sets `subscribed = true` only AFTER the response is flushed.
- **Tokio moved from dev-dep to non-dev dep on `pangolin-store`.** The WS recv loop in `sync_from_chain_with_ws_url` uses `tokio::time::timeout` + `tokio::time::Instant::now` + `tokio::time::sleep` to bound the recv loop. Tokio was previously dev-dep-only (used by hermetic async tests). The move introduces NO new external crate — tokio is already transitively in pangolin-store's dep tree via pangolin-chain. HIGH-1 + L7 invariants preserved (upstream crates unaffected by the directional move).
- **`tokio-tungstenite` + `futures-util` added as DEV-deps on `pangolin-chain`.** Both are transitively in the production tree via `alloy::providers::ws`, so the dev-dep declaration introduces NO new external crate. Pinned `=0.28.0` and `=0.3.32` respectively to the exact lockfile-resolved versions so a future alloy bump doesn't silently drift the mock-server's tree.
- **The `silent disconnect` test verifies alloy AUTO-RECONNECTS, not that the channel closes.** alloy's `WsConnect` default posture is 10 retries × 3-second base interval; a silent TCP drop triggers reconnection-not-close. The test asserts EITHER `connection_count >= 2` (alloy reconnected to the mock) OR a `SubscriptionClosed` recv outcome — both prove the silent disconnect was observed. Test budget set to 45s to cover the worst-case keepalive-detection window without timing out on slow CI.

**Open follow-ups.** (a) The Q-c Option III "JSON-file pin source-of-truth" arm is currently NOT wired into `sync_from_chain` — the resolver falls back to Option I (HTTP-URL scheme derivation) when no explicit override is passed. Pulling `chain.ws_default` through pangolin-store's deployment-file loader is a small mechanical follow-up; the L-ws-tls-downgrade defence fires inside `open_subscription` regardless of how the URL was obtained. (b) The host-side toggle of `SyncOptions.prefer_websocket = false` is covered by issue #100 (MVP-3-host-FFI-handles, next cycle in this batch). (c) Once D-017 sees its first `RevisionPublished` event on chain, the live `#[ignore]` residue test can be tightened to ASSERT a recv (not just a handle); today the assertion is "handle establishes" because there's no expected event yet.

Unblocks: closes the L8 deferral that 4.1 explicitly forecast as "MVP-3 4.1.x feature-flag flip." The WS-preferred branch is now end-to-end live; `SyncReport.event_source` reflects honestly whether tip-following ran via WS or fell back to HTTP. **This closes a §16-cycle in Kelvin's pre-MVP-3 cleanup batch** (4/5 done; only #100 remaining before MVP-3-host-FFI-handles).
