...
**Status: LOCKED — BUILD GO (option A, Kelvin 2026-05-20). Build now on the vetted-library design; NO external pre-opinion first (external audit unaffordable soon → in-house adversarial audit is the ONLY review before testnet → it must be exceptionally rigorous). Primitive-first 2-stage build: #104a = the threshold-crypto primitive in pangolin-crypto (build now); #104b = orchestration + the coupled anvil E2E (after #104a audited+merged). D-011 external audit stays a HARD pre-MAINNET gate — recovery is TESTNET-ONLY until it clears. Libraries: vsss-rs 5.4 + crypto_box 0.9.**

**Resolved foundational decisions (Kelvin 2026-05-20):** Q-c = **use a well-audited library** (composing vetted primitives, not inventing crypto). Q-d = the recovering device **sets a NEW password ONLY in true recovery** (lost-everything); a normal new-device add uses the EXISTING password (no new password) — keep the password-derived wrap-authority. Q-l = **ACCEPTED as a value proposition** (estate/inheritance recovery; "be your own guardians" by spreading shares across your own devices/locations) — the user controls trust via guardian choice + threshold; t-collusion reconstructing the VDK is the intended feature, and notably reconstruction is OFF-CHAIN so the 72h delay protects ownership-rotation, not VDK-confidentiality-against-a-quorum. Q-a (design beyond the under-specifying whitepaper) + Q-b (Scheme A — split the RWK) accepted on recommendation.
...
## 10. Auditor pre-opinion ask (the package to put in front of the D-011 firm)
...
- **Libraries to opine on:** `vsss-rs` 5.4 (Gf256, default-features off); `crypto_box` 0.9 (`seal`) — both Apache/MIT, RustCrypto/dalek ecosystem, same audited primitives as the existing `pangolin-crypto`.
...
**NEXT ACTION (Kelvin-owned): engage the D-011 external audit firm for this pre-opinion before the Workstream-B build is locked.** The build LOCK + dispatch waits on the pre-opinion.
