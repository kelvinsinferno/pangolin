...
- The new `extension-e2e` CI job green on `ubuntu-latest`. No Chrome (the gate injects a stdio host-bridge), but xvfb IS required: `pangolin-desktop` is a Tauri/GTK app whose IPC server only binds once its event loop initializes, which needs an X display — a displayless run never comes up. The job builds the desktop frontend dist first (the `custom-protocol` feature embeds it), builds the desktop with `test-hooks,custom-protocol` and the host **plain** (the host has no `test-hooks` feature; passing it leaves a stale, handshake-incompatible host binary), then runs `xvfb-run --auto-servernum pnpm test:e2e`.
